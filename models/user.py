...
class User(Base, IdentifiedMixin, TimestampMixin):
    ...
    __table_args__ = (
        ...
        tenant_scoped_unique("user_document", "document"),
    ...
    document: Mapped[str | None] = mapped_column(String(11), nullable=True)
    ...
    # What an address answers with to say it is the address, and when the last one was written to it.
    confirmation_token: Mapped[str | None] = mapped_column(String(64), nullable=True)
    confirmation_sent_at: Mapped[datetime | None] = mapped_column(UtcDateTime, nullable=True)
