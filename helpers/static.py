...
def setup_media(app: FastAPI) -> None:
    if settings.storage.provider is not StorageProvider.FILESYSTEM:
        return
...
def setup_admin(app: FastAPI) -> None:
    ...
    async def admin_entry(path: str = "") -> Response:
        ...
        # A last segment carrying a dot is a file being asked for, and answering the document there hands a browser HTML where it asked for a stylesheet.
        if "." in path.rsplit("/", 1)[-1]:
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND)

        # The document names the build by a hashed file, so one kept from an older build asks for assets that are gone.
