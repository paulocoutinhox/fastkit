...
def _form_fields(schema):
    return [field for fieldset in schema["form"]["fieldsets"] for field in fieldset["fields"]]
...
async def test_navigation_is_grouped(client, login):
    ...
    assert {"tenants", "users", "roles"} <= {item["resource"] for item in system["items"]}
    operations = next(group for group in groups if group["key"] == "operations")
    assert {"scheduled-tasks", "task-runs", "report-runs"} <= {item["resource"] for item in operations["items"]}
...
async def test_permission_flags_for_staff(client, login):
    ...
    catalog = (await client.get("/api/resources/showcase/schema")).json()["data"]["grid"]["flags"]

    assert catalog["can_create"] is True
    assert catalog["can_delete"] is True

    roles = (await client.get("/api/resources/roles/schema")).json()["data"]["grid"]["flags"]
...
async def test_product_crud_and_locale_decimal(client, login):
    ...
    categories = (await client.get("/api/resources/products/options/category_id")).json()["data"]
    category_id = categories[0]["value"]

    created = await client.post(
        "/api/resources/products",
        json={"name": "Add-on", "sku": "SKU-100", "price": "1.234,50", "category_id": category_id, "is_active": "true"},
        headers={"Accept-Language": "pt"},
    )
    assert created.status_code == 201
    identifier = created.json()["data"]["id"]

    detail = await client.get(f"/api/resources/products/{identifier}", headers={"Accept-Language": "pt"})
...
async def test_relation_and_dependent_options(client, login):
    ...
    category_id = next(entry["value"] for entry in categories if entry["label"] == "Premium")

    # subcategory options are empty until a parent category is chosen
    without_parent = (await client.get("/api/resources/products/options/subcategory_id")).json()["data"]
    assert without_parent == []

    dependent = (await client.get(f"/api/resources/products/options/subcategory_id?category_id={category_id}")).json()["data"]
...
async def test_validation_error_has_translated_message_text(client, login):
    ...
    response = await client.post("/api/resources/products", json={"sku": "NO-NAME"}, headers={"Accept-Language": "pt"})
    body = response.json()
...
async def test_fastapi_body_validation_is_translated(client):
    response = await client.post("/api/auth/login", json={"identifier": "x@y.com"}, headers={"Accept-Language": "pt"})
    body = response.json()

    assert response.status_code == 422
    password_error = [error for error in body["errors"] if error["field"] == "password"][0]
...
async def test_lookup_search_and_preload(client, login):
    ...
    matches = (await client.get("/api/resources/showcase/options/category_id?q=Prem")).json()["data"]
    assert {entry["label"] for entry in matches} == {"Premium"}

    value = matches[0]["value"]
    preloaded = (await client.get(f"/api/resources/showcase/options/category_id?value={value}")).json()["data"]
...
async def test_product_form_is_grouped_into_fieldsets(client, login):
    ...
    classification = next(fieldset for fieldset in schema["form"]["fieldsets"] if fieldset["title"] == "Classification")
    dependent = next(field for field in classification["fields"] if field["name"] == "subcategory_id")
...
async def test_bulk_action(client, login):
    ...
    response = await client.post("/api/resources/products/actions/deactivate", json={"ids": ids})
...
async def test_showcase_all_fields(client, login):
    ...
    assert {"text", "textarea", "richtext", "number", "decimal", "select", "multiselect", "lookup", "url", "email", "masked", "color", "boolean", "date", "time", "datetime", "json", "image", "file"} <= field_types
...
async def test_showcase_publish_action(client, login):
    ...
    response = await client.post("/api/resources/showcase/actions/publish", json={"ids": ids})
...
async def test_role_permission_editor(client, login):
    ...
    matrix = next(field for field in _form_fields({"form": schema["form"]}) if field["type"] == "permission_matrix")
    ...
    await client.put(f"/api/roles/{role_id}/permissions", json={"permission_ids": [first_permission]})
...
async def test_activity_log_records_actions_and_is_read_only(client, login):
    ...
    kept = await client.post("/api/resources/categories", json={"name": "Persisted", "is_active": "true"})
    kept_id = kept.json()["data"]["id"]

    created = await client.post("/api/resources/categories", json={"name": "Audited", "is_active": "true"})
    ...
    kept_row = next(row for row in logs if row["resource_type"] == "categories" and row["resource_id"] == "Persisted")
    assert kept_row["user_id"] == "Root"
    assert not any(row["resource_id"] == kept_id for row in logs if row["resource_type"] == "categories")

    flags = (await client.get("/api/resources/activity/schema")).json()["data"]["grid"]["flags"]
...
async def test_tenants_tasks_and_reports_resources(client, login):
    ...
    assert "succeeded" in " ".join(statuses) or any("succeeded" in status for status in statuses)
    ...
    scheduled = (await client.get("/api/resources/scheduled-tasks?sort=name")).json()["data"]
    assert {row["name"] for row in scheduled} >= {"Hourly sync", "Nightly cleanup"}

    running = (await client.get("/api/resources/task-runs?filter[status]=running")).json()["data"]
    assert [row["task_name"] for row in running] == ["demo.sync"]
    leased = (await client.get("/api/resources/task-runs?filter[status]=leased")).json()["data"]
    assert leased == []
...
async def test_viewer_is_denied_on_mutations_and_other_resources(client, login):
    ...
    assert (await client.post("/api/resources/products", json={"name": "x", "sku": "y", "price": "1"})).status_code == 403
    assert (await client.get("/api/resources/users/1")).status_code == 403
    assert (await client.post("/api/resources/task-runs/actions/enqueue_email", json={"ids": []})).status_code == 403
...
async def test_reports_run_and_export(client, login):
    ...
    assert [column["key"] for column in run["columns"]] == ["category", "products", "total"]
    ...
    options = (await client.get("/api/reports/sales-by-category/options/category_id")).json()["data"]
    premium_id = next(entry["value"] for entry in options if entry["label"] == "Premium")

    filtered = (await client.get(f"/api/reports/sales-by-category/run?category_id={premium_id}")).json()["data"]
    assert [row["category"] for row in filtered["rows"]] == ["Premium"]

    csv_export = await client.get(f"/api/reports/sales-by-category/export.csv?category_id={premium_id}")
...
async def test_enqueue_task_action(client, login):
    ...
    response = await client.post("/api/resources/task-runs/actions/enqueue_email", json={"ids": []})
    assert response.status_code == 200
    assert response.json()["data"]["enqueued"]

    runs = (await client.get("/api/resources/task-runs?sort=-created_at")).json()["data"]
...
async def test_tenant_crud(client, login):
    ...
    created = await client.post("/api/resources/tenants", json={"name": "Initech", "code": "initech", "status": "active", "is_active": "true", "default_locale": "en", "timezone": "UTC"})
    assert created.status_code == 201
...
async def test_content_manager_per_language(client, login):
    ...
    created = await client.post("/api/resources/content", json={"key": "home.hero", "type": "html"})
    content_id = created.json()["data"]["id"]

    await client.put(
        f"/api/content/{content_id}/translations",
        json={"translations": [{"language": "en", "body": "<p>Welcome</p>"}, {"language": "pt", "body": "<p>Bem-vindo</p>"}, {"language": "de", "body": "<p>ignored</p>"}]},
    )

    stored = (await client.get(f"/api/content/{content_id}/translations")).json()["data"]["translations"]
    ...
    english = (await client.get("/api/content-by-key/home.hero?language=en")).json()["data"]
    assert english["body"] == "<p>Welcome</p>"
    portuguese = (await client.get("/api/content-by-key/home.hero?language=pt")).json()["data"]
...
async def test_content_form_exposes_translations_editor(client, login):
    ...
    editor = next(field for field in _form_fields({"form": schema["form"]}) if field["type"] == "translations")
...
async def test_non_staff_cannot_access_admin_panel(client, demo, login):
    ...
    response = await login(client, email="member@fastkit.local", password="member-password-123")
...
async def test_profile_flow(client, login):
    ...
    updated = await client.put("/api/profile", json={"display_name": "Root Admin", "timezone": "Europe/Lisbon"})
    assert updated.json()["data"]["display_name"] == "Root Admin"

    password = await client.post("/api/profile/password", json={"current_password": "root-password-123", "new_password": "brand-new-password-1"})
    assert password.json()["message"]["code"] == "profile.password_changed"

    added = await client.post("/api/profile/identifiers", json={"type": "phone", "value": "+5511999998888"})
...
async def test_profile_avatar_and_upload(client, login):
    ...
    avatar = await client.post("/api/profile/avatar", files={"file": ("a.png", _png_bytes(), "image/png")})
    assert avatar.json()["data"]["url"].startswith("/media/")

    image = await client.post("/api/uploads/image", files={"file": ("editor.png", _png_bytes(), "image/png")})
    assert image.json()["data"]["url"].startswith("/media/")

    document = await client.post("/api/uploads/file", files={"file": ("notes.txt", b"hello", "text/plain")})
    assert document.json()["data"]["url"].startswith("/media/")

    unknown = await client.post("/api/uploads/other", files={"file": ("x.bin", b"x", "application/octet-stream")})
...
async def test_deleting_user_cascades_roles(client, login, demo):
    ...
    staff_id = next(user["id"] for user in users if user["email"] == "staff@fastkit.local")

    database = runtime.component("database")

    async with database.session_factory() as session:
        before = (await session.execute(select(func.count()).select_from(UserRole))).scalar_one()

    await client.delete(f"/api/resources/users/{staff_id}")

    async with database.session_factory() as session:
        after = (await session.execute(select(func.count()).select_from(UserRole))).scalar_one()
...
async def test_in_process_worker_runs_enqueued_task(monkeypatch, tmp_path):
    ...
    monkeypatch.setenv("FASTKIT__DATABASE__URL", f"sqlite+aiosqlite:///{tmp_path}/worker.db")
    ...
    try:
        ...
        execution = await queue.enqueue("demo.send_welcome_email", payload={"template": "welcome"}, queue="emails")
