...
async def test_a_token_signed_by_somebody_else_is_not_a_token(app, member, tenant_headers):
    forged = jwt.encode({"sub": member.token, "role": "administrator", "epoch": member.session_epoch}, "another-key-nobody-here-ever-signed-with", algorithm="HS256")
...
async def test_signing_up_puts_somebody_where_they_were_going_before_they_had_an_account(site, db, tenant):
    """The letter is the only thing that crosses the wait for the address to answer, so where the person was going travels inside it."""
    from tests.conftest import opened
    ...
    written = await db.scalar(select(OutboundEmail).where(OutboundEmail.to_address == "ada@acme.com"))
    opening = written.context["link"]

    assert opening.endswith("?next=%2Faccount%2Faddress")

    walked = urlsplit(opening)
    arrived = await site.get(f"{walked.path}?{walked.query}", follow_redirects=False)

    assert arrived.headers["location"] == "/account/address"
