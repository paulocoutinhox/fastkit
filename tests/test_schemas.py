...
@pytest.mark.parametrize("payload", [{"document": None}, {"document": ""}])
def test_an_empty_document_is_stored_as_nothing(payload):
...
def test_a_document_is_stored_without_punctuation():
    request = SignUpRequest(username="newcomer", password="s3cret-password", document="529.982.247-25")

    assert request.document == "52998224725"


def test_an_invalid_document_is_refused():
    with pytest.raises(ValidationError):
        SignUpRequest(username="newcomer", password="s3cret-password", document="12345678900")
...
def test_the_account_update_leaves_out_what_nobody_named():
    """The route writes only what was set, so a field left out keeps whatever the row already answers by."""
    assert AccountUpdateRequest().model_dump(exclude_unset=True) == {}


@pytest.mark.parametrize("field", ["timezone", "gender"])
def test_the_account_update_refuses_to_empty_a_column_the_row_must_have(field):
    """Asking to clear one of these is asking the table to hold a nothing it declared it never holds, and the database answers that with the refusal it keeps for a duplicate."""
    with pytest.raises(ValidationError):
...
@pytest.mark.parametrize("payload,expected", [({"document": "529.982.247-25"}, "52998224725"), ({"document": ""}, None)])
def test_the_user_payload_normalizes_the_document(payload, expected):
    assert UserCreate(username="newcomer", password="s3cret-password", email="newcomer@acme.com", **payload).document == expected


def test_the_user_payload_refuses_an_invalid_document():
    with pytest.raises(ValidationError):
        UserCreate(username="newcomer", password="s3cret-password", document="12345678900")
