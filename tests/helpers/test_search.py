...
def every_service(base=CrudService) -> list[type]:
    """The whole tree and not one level of it, because a resource with a base of its own — a tagged one, a localized one — drops out of a single call and is never checked again."""
...
def test_a_service_reading_prose_is_answered_by_an_index_covering_exactly_it():
    ...
    reading = [service for service in every_service() if service.text_search_fields]

    assert len(reading) >= 14, f"the scan found only {len(reading)} services reading prose, so it is proving nothing"

    for service in reading:
