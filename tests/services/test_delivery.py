...
async def test_a_grant_another_node_took_is_stepped_over(db, subscribed, member, monkeypatch, currency):
    """The grant was failed when the pass read it and taken when it got there, which is what losing the claim looks like — and a test that only drove the select would pass against a pass that never claimed."""
    await make_benefit(db, subscribed["entitlement"], type=BenefitType.CREDIT, currency_id=currency.id, target="gold", quantity=10)
    ...
    async def taken(session, grant_id):
        return False

    monkeypatch.setattr(delivery_service, "claim", taken)

    assert await delivery_service.retry_failed_grants(db) == []
    assert await balance_of(db, member, currency) == 0


async def test_a_grant_is_taken_before_it_is_worked_so_two_passes_never_work_one_twice(db, subscribed, member, monkeypatch, currency):
    """Both passes read the same grant, and only the one that took it delivers: without the claim both would, and each would burn one of the attempts the retry budget promises."""
    await make_benefit(db, subscribed["entitlement"], type=BenefitType.CREDIT, currency_id=currency.id, target="gold", quantity=10)
    ...
    assert await delivery_service.claim(db, grant.id) is True
    assert await delivery_service.claim(db, grant.id) is False
...
async def test_a_cycle_another_node_already_claimed_is_read_instead_of_erroring(db, subscribed, currency):
    ...
    settled = await delivery_service.settled(db, twin, grant.grant_key)
...
async def test_a_cycle_the_node_lost_is_not_delivered_twice(db, subscribed, monkeypatch, currency):
    ...
    monkeypatch.setattr(type(delivery_service), "settled", lost)
...
async def test_a_benefit_another_activation_already_snapshot_is_read_instead_of_erroring(db, subscribed, monkeypatch, currency):
    ...
    async def blind_once(self, session, target):
        ...
        # The row the other activation writes is the one the snapshot writes, currency and all, or this drives a state the product refuses to create.
        session.add(
            SubscriptionBenefit(subscription_id=target.id, benefit_id=benefit.id, user_entitlement_id=entitlement.id, currency_id=benefit.currency_id, status=BenefitStatus.ACTIVE, benefit_type=benefit.type, target=benefit.target, quantity=benefit.quantity, cadence=benefit.cadence, anchor_at=now(), next_grant_at=now())
        )
        await session.commit()
