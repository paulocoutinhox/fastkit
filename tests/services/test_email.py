...
def captured(monkeypatch):
    ...
    async def capture(config, letter):
        sent.append((config, letter))
...
async def test_the_message_is_html_carrying_what_the_context_gave_it(db, monkeypatch, captured):
    ...
    config, letter = captured[0]
    envelope = mailer.PROVIDERS[EmailProvider.SMTP].envelope(config, letter)

    assert "abc" in letter.html
    assert envelope["From"] == "Acme <no-reply@acme.com>"
    assert envelope.get_content_type() == "text/html"
...
async def test_a_tenant_sends_through_its_own_configuration(db, tenant, monkeypatch, captured):
    ...
    assert config.from_address == "no-reply@own.com"
...
async def test_the_console_provider_writes_instead_of_dialling_out(db, caplog):
    """Nothing here stubs the mailer, so the message walks the real provider — and a suite that cannot open a socket is what proves nothing was dialled."""
    await queued(db)
    ...
    assert "no-reply@localhost" in caplog.text
...
async def test_the_delivery_hands_the_configuration_to_the_smtp_client(monkeypatch):
    ...
    await mailer.send(SMTP, mailer.Letter(to="reader@acme.com", subject="Olá", html="<p>corpo</p>"))

    assert calls[0] == {"hostname": "smtp.acme.com", "port": 2525, "username": "robot", "password": "s3cret", "start_tls": False, "timeout": mailer.TIMEOUT}
...
async def test_credentials_left_empty_reach_the_client_as_nothing(monkeypatch):
    ...
    await mailer.send(anonymous, mailer.Letter(to="reader@acme.com", subject="Olá", html="<p>corpo</p>"))
...
async def test_the_file_of_the_tenant_wins_over_the_shared_one(db, tenant, monkeypatch, captured, tmp_path):
    ...
    assert "proprio abc" in captured[0][1].html
...
async def test_what_the_tenant_does_not_carry_comes_from_the_shared_folder(db, tenant, monkeypatch, captured, tmp_path):
    ...
    assert "partilhado abc" in captured[0][1].html
...
async def test_one_message_that_cannot_be_settled_never_takes_the_pass_with_it(db, tenant, monkeypatch):
    ...
    monkeypatch.setattr(settings.email, "provider", EmailProvider.CONSOLE)
