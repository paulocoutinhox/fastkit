...
def test_a_gateway_whose_authenticate_is_a_coroutine_is_refused():
    """The caller does not wait for it, so one written this way hands back an object nobody reads: it never runs, it never raises, and the webhook is open to whoever finds the key."""
    from services.gateway import Credential, PaymentProvider, ProviderEvent

    with pytest.raises(TypeError, match="does not wait for one"):

        class Sleepy(PaymentProvider):
            event_stated = True
            credentials = (Credential("stripe_webhook_secret", "Signing secret", "Developers, webhooks"),)

            async def authenticate(self, integration, call, secret):
                raise AssertionError("this is the line a gateway counts on, and it would never be reached")

            async def read(self, integration, call, client):
                return ProviderEvent(external_event_id="x", event_type="y")


def test_a_gateway_whose_read_is_not_a_coroutine_is_refused():
    """The caller waits for what these two answer, and waiting on a value that is not a coroutine is a failure inside a request."""
    from services.gateway import Credential, PaymentProvider, ProviderEvent

    with pytest.raises(TypeError, match="without async"):

        class Flat(PaymentProvider):
            event_stated = True
            credentials = (Credential("stripe_webhook_secret", "Signing secret", "Developers, webhooks"),)
            ...
            def read(self, integration, call, client):
                return ProviderEvent(external_event_id="x", event_type="y")
