...
def test_every_model_a_deletion_walks_into_is_served_by_a_service(app):
    """The map is what says which files a child mentioned, so a child nothing serves is a deletion that empties the row and leaves its files in the bucket."""
    from services.crud import referenced, services_by_model

    served = services_by_model()
    unserved, read = [], 0

    for service in {type(value) for value in served.values()}:
        for dependent in service.dependents:
            read += 1

            if dependent.model not in served:
                unserved.append(f"{service.__name__} deletes {dependent.model.__name__} and no service says what it mentions")

    # The other reader of the map is the one that narrows a foreign key to what the operator reaches.
    for model in served:
        for name, target in referenced(model).items():
            read += 1

            if target not in served:
                unserved.append(f"{model.__name__}.{name} points at {target.__name__} and no service says who reaches it")

    assert read >= 27, f"the scan read only {read} of them, so it is proving nothing"
    assert unserved == [], f"the map is read by index, and these would raise: {unserved}"
