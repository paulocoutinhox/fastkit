...
async def test_a_worker_that_fell_over_says_so_instead_of_keeping_it_to_itself(caplog):
    """A node whose cron died answers every request exactly like one with nothing to do, so the only sign of it is what this writes."""

    async def falls_over():
        raise RuntimeError("the queue went away")

    polling = asyncio.create_task(falls_over())

    with pytest.raises(RuntimeError):
        await polling

    with caplog.at_level(logging.ERROR):
        lifespan.announce_a_worker_that_stopped(polling)

    assert "nothing is scheduled on this node" in caplog.text
    assert "the queue went away" in caplog.text


@pytest.mark.parametrize("stopping", ["finished", "cancelled"])
async def test_a_worker_that_was_asked_to_stop_says_nothing(caplog, stopping):
    """A deploy stops the worker on purpose, and an error written on every restart is one nobody reads by the third time."""

    async def waits():
        await asyncio.sleep(60)

    polling = asyncio.create_task(asyncio.sleep(0) if stopping == "finished" else waits())
...
async def test_a_file_of_the_build_that_is_gone_is_not_answered_with_the_document(tmp_path, monkeypatch):
    """A stylesheet answered with the entry point is html where a browser asked for css, and the panel draws with no style and no error saying why."""
    application = build_dists(tmp_path, monkeypatch)

    async with AsyncClient(transport=ASGITransport(app=application), base_url="http://test") as client:
        gone = await client.get("/admin/assets/index-fromanolderbuild.css")
        entry = await client.get("/admin")

        assert gone.status_code == 404, "a stylesheet that is gone was answered with the document"
        assert (await client.get("/admin/products/1/edit")).status_code == 200, "a route of the panel is not a file and still answers the document"
        assert entry.headers["cache-control"] == "no-cache", "the document names a hashed build, so a browser has to ask whether it moved"
...
def test_the_application_under_test_is_wired_the_way_the_real_one_is():
    ...
    root = named(pathlib.Path("app.py").read_text())
    suite = named(pathlib.Path("tests/conftest.py").read_text())

    assert len(root) >= 6, "the scan read almost nothing out of app.py, so it is proving nothing"
    ...
    assert wiring.search(pathlib.Path("app.py").read_text()), "app.py stopped handing the error layer the page the site draws"
...
def test_every_closed_set_the_code_dispatches_on_answers_for_every_value():
    ...
    assert len(tables) >= 13, f"the scan found only {len(tables)} of them, so it is proving nothing"
...
def test_every_cookie_this_site_writes_carries_the_name_of_this_product():
    """This repository is a template, so a product started from it that renamed itself would otherwise write another product's name on the browser of every reader it has."""
    import pathlib
    import re

    from config.base import NAME, SLUG
    from helpers.settings import settings
    from helpers.text import slugify

    assert SLUG == slugify(NAME), f"the identifier of this installation is {SLUG} and its name reduces to {slugify(NAME)}"

    holders = [settings.site, settings.site.consent]
    named = [(f"{type(holder).__name__}.{field}", getattr(holder, field)) for holder in holders for field in type(holder).model_fields if field.endswith("cookie")]

    assert len(named) >= 7, f"the scan found {len(named)} cookie names, so it is proving nothing"

    for where, value in named:
        assert value.startswith(f"{SLUG}_"), f"{where} is {value}, which is not a cookie of {NAME}"

    # A cookie spelled anywhere but here is one a rename leaves behind, and the seeded policy that names them for a reader is read the same way.
    spelled = [str(path) for path in pathlib.Path().glob("*/*.py") if path.parts[0] != "tests" and re.search(rf'"[^"]*{SLUG}_\w+', path.read_text())]

    assert spelled == [], f"these write a cookie name of their own instead of reading the one the configuration declares: {spelled}"
...
def test_the_panel_turns_its_palette_by_the_choice_and_not_by_the_machine():
    """The theme button writes a `color-scheme`, and a build that transpiles `light-dark()` away leaves nothing reading it: the palette answers the system preference and pressing the button does nothing."""
    import pathlib

    built = sorted(pathlib.Path("webapps/admin/dist/assets").glob("*.css"))

    assert len(built) == 1, "the panel has not been built, and this measures what a browser is given"

    written = built[0].read_text()

    assert written.count("light-dark(") > 20, "the panel stopped drawing with `light-dark()`, which is the whole of how it carries two palettes"
    assert "--lightningcss-" not in written, "`light-dark()` was transpiled into variables only `prefers-color-scheme` switches, so the theme button turns nothing"
...
def test_every_schema_this_project_builds_is_built_from_the_one_list():
    ...
    named = []
    read = [path for path in sorted(pathlib.Path().glob("*/*.py")) if "tests" not in path.parts]

    assert len(read) >= 100, f"the scan read only {len(read)} files, so it is proving nothing"
