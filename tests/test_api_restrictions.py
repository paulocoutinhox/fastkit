...
# A document and a phone number arrive punctuated and are stored stripped, so the schema accepts more characters than the column keeps.
PUNCTUATED = {("UserCreate", "document"), ("UserUpdate", "document"), ("SignUpRequest", "document"), ("UserCreate", "mobile_phone"), ("AccountUpdateRequest", "mobile_phone")}
...
def update_schemas() -> list[tuple[str, type, type]]:
    """Every edit payload paired with the model it writes into, read off the module that declares it because `as_optional` builds the class somewhere else."""
    import models.registry  # noqa
    ...
    for path in sorted(pathlib.Path("schemas").glob("*.py")):
        ...
        for name in dir(module):
            model = models.get(name.removesuffix("Update")) if name.endswith("Update") else None

            if model is not None:
                found[name] = (name, getattr(module, name), model)
...
def columns_a_service_fills_in_for_itself() -> set[tuple[str, str]]:
    """The value a record is addressed by is derived from its title when nobody typed one, so naming it null is asking for that and never asking to store a nothing."""
    found = set()

    for path in sorted(pathlib.Path("services").glob("*.py")):
        found |= {(path.stem, target) for target in re.findall(r'apply_slug\(.+?, "(\w+)", \(', path.read_text())}

    assert len(found) >= 6, f"the scan found only {len(found)} derived columns, so it is proving nothing"

    return found


@pytest.mark.parametrize("name, schema, model", update_schemas(), ids=lambda value: value if isinstance(value, str) else "")
def test_no_edit_payload_offers_to_write_a_nothing_the_column_has_nowhere_to_put(name, schema, model):
    """Leaving a field out is how an edit keeps what is there, and naming it null asks the table for a nothing it declared it never holds, which the database refuses with the answer it keeps for a duplicate."""
    derived = columns_a_service_fills_in_for_itself()
    loose = []

    for field, definition in schema.model_fields.items():
        column = model.__table__.columns.get(field)

        if column is None or column.nullable or (model.__module__.removeprefix("models."), field) in derived:
            continue

        if type(None) in typing.get_args(definition.annotation):
            loose.append(field)

    assert loose == [], f"{name} accepts a null where {model.__tablename__} declares a value: {loose}"


def test_the_edit_payloads_are_the_ones_the_resources_are_written_by():
    """A pairing that stopped matching leaves the check above running over nothing and passing for ever."""
    paired = update_schemas()

    assert len(paired) >= 18, f"the scan paired only {len(paired)} edit payloads, so it is proving nothing"
