...
def declared_builders() -> list[str]:
    """The vocabulary a resource is declared with, read off the file that declares it, because a list written here by hand is one that quietly stops covering a kind of field."""
    found = sorted(set(re.findall(r"^export function (\w+)\(", pathlib.Path("webapps/admin/src/resources/fields.js").read_text(), re.M)))

    assert len(found) >= 15, f"the scan read only {len(found)} builders, so it is proving nothing"

    return found


def matching(names) -> str:
    return rf"\b(?:{'|'.join(names)})\(\s*"


DRAWN_FIELDS = matching(declared_builders())

# A read only field is drawn and never sent, so it answers to no write payload.
FIELD_BUILDERS = matching(name for name in declared_builders() if name != "readOnly")
...
def test_every_field_the_admin_draws_is_a_field_the_api_answers(app):
    ...
    for source in sorted(pathlib.Path("webapps/admin/src/resources").glob("*.js")):
        ...
        for block in re.findall(r"export const \w+ = \{(.*?)\n\};", source.read_text(), re.S):
            ...
            fields = set(re.findall(r'\{\s*name:\s*"(\w+)"', block)) | set(re.findall(DRAWN_FIELDS + r'"(\w+)"', block))
...
def test_the_profile_screen_writes_only_what_the_account_accepts():
    """The own account is not a resource, so its screen names its fields by hand, and what it writes goes to the two payloads an account sends about itself: every field it draws has to be one of them, and every one of them has to be drawn."""
    from schemas.auth import AccountUpdateRequest, PasswordChangeRequest

    screen = pathlib.Path("webapps/admin/src/views/ProfileView.vue").read_text()
    drawn = set(re.findall(rf"{FIELD_BUILDERS}\"(\w+)\"", screen))
    accepted = {definition.alias for schema in (AccountUpdateRequest, PasswordChangeRequest) for definition in schema.model_fields.values()}

    assert len(drawn) >= 9, f"the scan read only {len(drawn)} fields of the screen, so it is proving nothing"
    assert sorted(drawn - accepted) == [], "the profile screen draws what the account API does not accept"

    # A field the account accepts and the screen never draws is one nobody can change from the panel, and the language is chosen in the bar at the top.
    assert sorted(accepted - drawn) == ["languageId"]


# What a label leaves in lower case, which is the small words of each language and nothing else.
SMALL_WORDS = {"en": {"a", "an", "the", "and", "or", "of", "in", "on", "at", "to", "for", "by", "with", "from"}, "pt": {"a", "o", "as", "os", "de", "do", "da", "dos", "das", "e", "em", "no", "na", "nos", "nas", "por", "para", "com"}, "es": {"a", "el", "la", "los", "las", "de", "del", "y", "en", "por", "para", "con"}}


def test_every_resource_is_labelled_with_the_case_the_others_carry():
    """A menu is read as a column, so the one entry that dropped a word to lower case reads as the one somebody forgot to finish."""
    wrong = []
    checked = 0

    for catalog in catalogs():
        text = pathlib.Path(catalog).read_text()
        body = block_of(text, text.index("{", text.index("\n    resource: ")))
        small = SMALL_WORDS[pathlib.Path(catalog).stem]

        for label in re.findall(r'(?:menu|title|singular): "([^"]+)"', body):
            checked += 1
            words = label.split()
            loose = [word for position, word in enumerate(words) if word[0].islower() and (position == 0 or word.lower() not in small)]

            if loose:
                wrong.append(f"{catalog}: {label!r} leaves {loose} in lower case")

    assert checked >= 270, f"the scan read only {checked} labels, so it is proving nothing"
    assert wrong == [], f"these do not carry the case every other label carries: {wrong}"
