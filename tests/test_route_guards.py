...
# Neither a brand nor an optional account guards anything: one says which site is answering, and the other reads whoever is there when nobody has to be.
ANNOTATED = {"AdministratorUser": "administrator", "CurrentUser": "account"}

DEPENDED = {"get_administrator": "administrator", "get_current_user": "account"}
...
OPEN = {
    ...
    ("POST", "/confirmation"): "an account waiting on its address has no session to ask with, which is the whole point of it waiting",
    ("POST", "/confirmation/{token}"): "the token mailed to the address is the credential, and answering it is what opens the account",
    ("GET", "/active"): "the languages offered are public, and so are the banners and the galleries running now",
    ("GET", "/by-tag/{tag}"): "a page of content and a gallery are drawn for whoever opens the address, which is what the site exists to be found by",
    ("GET", "/plans"): "the plans on sale are what an application puts on its own paywall, and the site draws the same list",
    ("POST", "/{uuid}/view"): "a banner is counted by a reader with no account, and the name it is counted by is one this side signed",
    ("POST", "/{uuid}/click"): "the same reader following the same banner",
    ("GET", "/offered"): "the countries an address may name are public, and the postal code of one is not",
    ("POST", ""): "writing to the operator and joining a newsletter are done by whoever has no account and both carry a challenge, and an application reports what it did before anybody has signed in",
    ("GET", "/products"): "the catalogue is what the site draws and an application lists, and what an account already owns is filled in afterwards",
    ("GET", "/products/{slug}"): "the page of one product, read by whoever opens the address",
...
NOT_A_RECORD = {
    ...
    ("POST", "/confirmation/{token}"): "the token is the credential, and it is what the account is found by",
}
