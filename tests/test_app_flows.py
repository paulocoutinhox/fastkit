...
async def signed_up(client, db, headers: dict, email: str) -> str:
    """Signing up and then answering the address, because where the environment asks for that the two together are the journey."""
    created = await client.post("/api/signup", json={"email": email, "password": "a-strong-secret"}, headers=headers)
    ...
    return (await client.post(f"/api/account/confirmation/{account.confirmation_token}")).json()["token"]
...
async def test_somebody_signs_up_answers_the_address_and_lands_in_the_account(client, tenant_headers, db):
    """The first screen: two buttons, and this is the one that creates the account — which then waits on the address it named."""
    ...
    # The account is there and it is waiting, so no session came back with it.
    assert created.json()["token"] is None
    assert created.json()["user"]["status"] == UserStatus.PENDING

    account = await db.scalar(select(User).where(User.email == "new@acme.com"))
    answered = await client.post(f"/api/account/confirmation/{account.confirmation_token}")

    assert answered.status_code == 200

    me = await client.get("/api/account/me", headers={"Authorization": f"Bearer {answered.json()['token']}"})

    assert me.status_code == 200
    assert me.json()["email"] == "new@acme.com"


async def test_signing_in_answers_the_same_account_the_signup_made(client, tenant_headers, db):
    await signed_up(client, db, tenant_headers, "back@acme.com")
...
async def test_the_address_of_somebody_else_is_never_reached(client, db, tenant, member, member_headers, tenant_headers):
    ...
    session = {"Authorization": f"Bearer {await signed_up(client, db, tenant_headers, 'stranger@acme.com')}"}
...
async def test_the_person_edits_the_account_and_cannot_erase_the_last_way_in(client, member_headers, member):
    ...
    refused = await client.put("/api/account/me", json={"email": "", "username": "", "document": "", "mobilePhone": ""}, headers=member_headers)
...
async def test_a_subscription_of_somebody_else_answers_nothing(client, subscriber, db, tenant, storefront):
    session = {"Authorization": f"Bearer {await signed_up(client, db, {'X-Tenant-Code': tenant.code}, 'another@acme.com')}"}
