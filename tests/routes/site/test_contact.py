...
async def test_the_operator_can_answer_the_person_who_wrote(site, db):
    ...
    queued = await db.scalar(select(OutboundEmail))
    envelope = mailer.PROVIDERS[EmailProvider.SMTP].envelope(settings.email, mailer.Letter(to=queued.to_address, subject=queued.subject, html="<p>body</p>", reply_to=queued.reply_to))
...
async def test_a_message_nobody_answers_carries_no_reply_address(db, tenant):
    ...
    assert mailer.PROVIDERS[EmailProvider.SMTP].envelope(settings.email, mailer.Letter(to=queued.to_address, subject=queued.subject, html="<p>body</p>", reply_to=queued.reply_to))["Reply-To"] is None
