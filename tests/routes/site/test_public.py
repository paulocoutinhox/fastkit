...
async def test_the_language_that_was_chosen_outranks_the_one_the_browser_asked_for(site):
    site.cookies.set(settings.site.language_cookie, "en")
...
async def test_a_page_is_drawn_in_the_palette_the_browser_chose(site):
    ...
    site.cookies.set(settings.site.theme_cookie, "dark")
...
def test_the_surface_of_a_card_is_written_once():
    ...
    assert len(pages) >= 20, f"the scan read only {len(pages)} pages, so it is proving nothing"
...
def test_every_page_that_writes_reads_the_token_that_proves_where_the_form_came_from():
    """A cookie travels on its own, so a form of this site is told apart from a form of somebody else by a value only a page of ours can read."""
    import ast

    unguarded, counted = [], 0

    for path in sorted(pathlib.Path("routes/site").glob("*.py")):
        for node in ast.walk(ast.parse(path.read_text())):
            if not isinstance(node, (ast.FunctionDef, ast.AsyncFunctionDef)) or not node.decorator_list:
                continue

            if not re.search(r"\.post\(", " ".join(ast.unparse(one) for one in node.decorator_list)):
                continue

            counted += 1
            body = "\n".join(ast.unparse(line) for line in node.body)

            if "guard(" not in body:
                unguarded.append(f"{path}:{node.lineno} {node.name}")

    assert counted >= 15, f"the scan read only {counted} of them, so it is proving nothing"
    assert unguarded == [], f"these write and never ask where the form came from: {unguarded}"
