...
async def test_the_link_in_the_message_opens_the_account_and_lands_in_it(site, db, tenant):
    """The link is clicked in a mail client, so it is a GET, and answering it is what puts the person inside."""
    token = await opened(site, "/account/signup")
    ...
    answered = await site.get(f"/account/confirm/{account.confirmation_token}", follow_redirects=False)
