...
async def test_two_password_changes_that_crossed_each_end_the_session_the_other_opened(db, member):
    """A counter raised in Python loses one of two writes, and what is lost here is a session the change was meant to end."""
    from services.auth import auth_service

    opened = member.session_epoch

    await auth_service.settle_password(db, member, "uma-senha-nova")

    # The second caller came in holding the row as it was before the first one wrote, which is what two requests do.
    member.session_epoch = opened

    await auth_service.settle_password(db, member, "outra-senha-ainda")
    await db.refresh(member)

    assert member.session_epoch == opened + 2, "one of the two changes wrote an epoch the other had already used"
