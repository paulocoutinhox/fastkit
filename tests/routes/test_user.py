...
async def test_create_normalizes_the_email_and_the_document(client, admin_headers):
    response = await client.post("/api/users", json=build_payload(email="MiXeD@Acme.com", document="529.982.247-25"), headers=admin_headers)
...
async def test_update_refuses_clearing_every_identity(client, member, admin_headers):
    response = await client.put(f"/api/users/{member.id}", json={"username": None, "email": None, "document": None, "mobilePhone": None}, headers=admin_headers)
