...
async def test_an_event_is_taken_by_one_reader_and_the_other_is_told_it_is_taken(db, tenant, member):
    """The retry pass and a redelivery reach the same row, and reading it twice is what the claim exists to stop."""
    from services.webhook import webhook_service

    integration, _ = await wire(db, tenant, member)
    record = await save(db, WebhookEvent(tenant_id=tenant.id, integration_id=integration.id, external_event_id="evt-disputado", payload_hash="h", status=WebhookEventStatus.FAILED, payload=event("INITIAL_PURCHASE", member), meta={}))

    assert await webhook_service.claim(db, record.id) is True
    assert await webhook_service.claim(db, record.id) is False

    await db.refresh(record)

    assert record.status is WebhookEventStatus.PROCESSING
    assert record.attempts == 1


async def test_a_delivery_of_an_event_another_reader_is_holding_is_not_read_again(client, db, tenant, member):
    """A gateway redelivers what it is unsure about, and the pass may be inside that very row when it arrives."""
    integration, _ = await wire(db, tenant, member)
    body = json.dumps(event("INITIAL_PURCHASE", member)).encode()

    first = await client.post(f"/api/webhooks/{integration.webhook_key}", content=body, headers={"Content-Type": "application/json"})
    held = await db.scalar(select(WebhookEvent).where(WebhookEvent.integration_id == integration.id))

    assert first.status_code == 200

    # The row is put back the way a reader holding it looks, which is what a redelivery lands on.
    await db.execute(update(WebhookEvent).where(WebhookEvent.id == held.id).values(status=WebhookEventStatus.PROCESSING, attempts=1))
    await db.commit()

    again = await client.post(f"/api/webhooks/{integration.webhook_key}", content=body, headers={"Content-Type": "application/json"})
    await db.refresh(held)

    assert again.status_code == 200
    assert held.attempts == 1, "the delivery read a row somebody else was holding"
    assert held.status is WebhookEventStatus.PROCESSING


async def test_a_row_another_reader_took_between_the_read_and_the_claim_is_left_alone(db, tenant, member, monkeypatch):
    """The pass reads its queue once and claims one row at a time, so a row can already be gone by the time it asks for that one."""
    from services.webhook import webhook_service

    integration, _ = await wire(db, tenant, member)
    record = await save(db, WebhookEvent(tenant_id=tenant.id, integration_id=integration.id, external_event_id="evt-tomado", payload_hash="h", status=WebhookEventStatus.FAILED, payload=event("INITIAL_PURCHASE", member), meta={}))
    taking = webhook_service.claim

    async def by_somebody_else(session, record_id):
        # The other reader really takes it, so the row is left the way a race leaves it.
        await taking(session, record_id)

        return False

    monkeypatch.setattr(webhook_service, "claim", by_somebody_else)

    assert await webhook_service.retry_failed(db) == []
