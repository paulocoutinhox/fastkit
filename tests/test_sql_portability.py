...
# A counter this side raises in Python, and why losing one of two writes costs nothing where it does it.
RAISED_IN_PYTHON: dict[str, str] = {}

# Nothing in the running code raises one any more, so the scan proves it still recognises one on a line written here.
PROBE = "record.attempts += 1"


def counters_in(tree) -> list:
    """Every number this side reads and writes back, which is the shape that loses one of two writes."""
    import ast

    return [node for node in ast.walk(tree) if isinstance(node, ast.AugAssign) and isinstance(node.target, ast.Attribute) and isinstance(node.op, (ast.Add, ast.Sub))]


def test_no_counter_two_requests_share_is_raised_in_python():
    """Reading a number and writing it back loses one of two writes, and every one of these is a number two callers reach at once."""
    import ast
    ...
    for path in sources():
        ...
        for node in counters_in(ast.parse(path.read_text())):
            named = ast.unparse(node.target)

            if named not in RAISED_IN_PYTHON:
                raised.append(f"{path.relative_to(ROOT)}:{node.lineno} {ast.unparse(node)}")

    assert read >= 140, f"the scan read only {read} modules, so it is proving nothing"
    assert counters_in(ast.parse(PROBE)), "the scan no longer recognises a counter raised in python, so it would say nothing about one"
    assert raised == [], f"these are raised by this side, and the database is what has to raise them: {raised}"


def test_no_counter_is_excused_from_a_rule_it_no_longer_breaks():
    """An excuse that outlives the line it was written for is one nobody reads again, and it would cover the next counter to take that name."""
    written = "\n".join(path.read_text() for path in sources())
    stale = sorted(name for name in RAISED_IN_PYTHON if f"{name} +=" not in written and f"{name} -=" not in written)
