...
# The helper is where the one loop of a command is opened, and it is the engine it empties afterwards.
ALLOWED = ("helpers/db.py",)
...
def test_nothing_that_drives_the_shared_session_opens_a_loop_of_its_own():
    ...
    for path in sources():
        ...
        # The names are searched however the module spells its way to them, because reading one import form is reading half the code.
        if re.search(r"\b(AsyncSessionLocal|async_engine)\b", body) and "asyncio.run(" in body and path.as_posix() not in ALLOWED:
            driving.append(str(path))

    assert read > 140, f"the guard read {read} modules, so it is proving nothing"
