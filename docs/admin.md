...
account already answers for itself: `PUT /api/account/me` for the name, the nickname, the gender, the
email, the phone and the timezone, `POST` and `DELETE /api/account/avatar` for the picture, and
