...
## Every queue takes the row before it works it

Claiming the run settles which node fires a job. It does not settle which node works a given row, and
those are different questions: a pass that runs past its interval overlaps the next, and `make delivery`
calls the same functions outside the scheduler entirely.

So each of the four passes does the same thing — a conditional `UPDATE` moving the row into a working
state, gated on it still being in the waiting one, a commit per item, and a reclaim of whatever stayed
taken longer than the pass could plausibly hold it. The pass works only what it won.

| The pass | What two of them at once would otherwise do |
| --- | --- |
| the mail queue | dial the same message twice, and the reader receives it twice |
| the events the clients reported | write the same row of the record twice — measured at 719 rows for 120 events with the claim off |
| the notices a gateway sent | the live delivery and the recovery pass meet on the same row, because a gateway redelivers exactly what failed |
| the deliveries that failed | spend two of the attempts the configuration promises, so the cycle is given up before its fifth try |

The predicate is written once and read by both the select and the claim. Two spellings of *what may be
picked up* drift, and then a pass reads one thing and takes another.

The delivery pass is the one that least looks like it needs this. The main pass is already safe, but by
something else: it **creates** the grant, and the unique key on the cycle settles two passes creating the
same one. The retry works a row that already exists, where that key protects nothing.

What it costs to skip is worth stating plainly, because it is not always a double delivery. Here the
three benefit types are idempotent — credit by the key on the ledger, a product because owning is one
row, access because turning on what is on is not a second delivery — so nothing is handed over twice.
The claim is what makes the pass safe **by construction** rather than by a property of today's handlers,
which is what a fourth type would quietly break: one that adds to whatever is standing rather than to
today is not idempotent, and a second delivery of it is a second addition.
