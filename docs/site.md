...
/account/confirm/{token}   the address saying it is the address, and the one page under /account/ that opens a session instead of asking for one
/account/confirmation      asking for the letter again, for somebody who never got it and therefore cannot sign in
...
| 2 | the language cookie | the only place left for somebody with no account |
...
The consent has consequences, or it is decoration. The `language_cookie` is the only cookie of preference this
...
lives in the `theme_cookie`, so the server writes `data-theme` on the page and nothing ever draws
