...
from fastkit_cache.database.entry import CacheEntry
...
class DatabaseCacheProvider:
    ...
    async def increment(self, key: str, amount: int = 1, ttl: int | None = None) -> int:
        ...
        async with self._database.session_factory() as session:
            entry = await self._locked_entry(session, key_hash)

            if entry is not None:
                value = self._next_value(entry, amount, now, ttl)
                await session.commit()
            ...
            session.add(
                CacheEntry(
                    ...
                    key_hash=key_hash,
                    key=key,
                    value=_encode_int(amount),
                    expires_at=now + _seconds(ttl) if ttl is not None else None,
                )
            )
            ...
            try:
                ...
                return await self._increment_existing(key_hash, amount, now)

            return amount

    async def _increment_existing(
        self, key_hash: str, amount: int, now: datetime
    ) -> int:
        async with self._database.session_factory() as session:
            entry = await self._locked_entry(session, key_hash)
            value = self._next_value(entry, amount, now, None)
            await session.commit()

            return value

    async def _locked_entry(self, session, key_hash: str) -> CacheEntry | None:
        return (
            await session.execute(
                select(CacheEntry)
                .where(CacheEntry.key_hash == key_hash)
                .with_for_update()
            )
        ).scalar_one_or_none()

    def _next_value(
        self, entry: CacheEntry, amount: int, now: datetime, ttl: int | None
    ) -> int:
        if entry.expires_at is not None and _aware(entry.expires_at) <= now:
            entry.value = _encode_int(amount)
            entry.expires_at = now + _seconds(ttl) if ttl is not None else None

            return amount

        value = int(entry.value.decode("utf-8")) + amount
        entry.value = _encode_int(value)
