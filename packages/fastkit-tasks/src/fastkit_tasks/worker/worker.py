...
from fastkit_tasks.worker.context import TaskContext
...
class Worker:
    ...
    async def run_once(self) -> str | None:
        ...
        if execution.timeout_seconds >= self._lease_seconds:
            # extend the lease to outlast the task's own timeout so a long-running task is not
            # reclaimed and re-run concurrently by another worker while it is still executing
            lease_seconds = execution.timeout_seconds + self._lease_seconds
            await self._queue.heartbeat(execution.id, self._worker_id, lease_seconds)

        context = TaskContext(self._queue, execution, self._worker_id, lease_seconds)
