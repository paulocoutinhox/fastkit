...
class TaskQueue:
    ...
    async def enqueue(
        ...
        definition = (
            ...
            if self._registry is not None and self._registry.contains(task_name)
            else None
        )
        attempts = (
            max_attempts
            if max_attempts is not None
            else definition.max_attempts
    ...
    async def lease(
        self, worker_id: str, queues: list[str], lease_seconds: int = 60
    ) -> TaskExecution | None:
        now = self._clock()
        ...
        async with self._database.session_factory() as session:
            candidates = (
                (
                    await session.execute(
                        ...
                        .order_by(
                            TaskExecution.priority.desc(),
                            TaskExecution.available_at.asc(),
                        )
                        .limit(10)
                    )
                )
            ...
            for execution_id in candidates:
                locked = await self._try_lock(
                    session, execution_id, worker_id, now, lease_seconds
                )
    ...
    async def _try_lock(
        self, session, execution_id, worker_id, now, lease_seconds
    ) -> TaskExecution | None:
        result = await session.execute(
            ...
            .where(
                ...
                TaskExecution.status.in_(
                    [ExecutionStatus.pending.value, ExecutionStatus.retrying.value]
                ),
                TaskExecution.available_at <= now,
                or_(
                    TaskExecution.locked_until.is_(None),
                    TaskExecution.locked_until < now,
                ),
            )
            .values(
    ...
    async def heartbeat(
        ...
        async with self._database.session_factory() as session:
            ...
            result = await session.execute(
                ...
                .where(
                    TaskExecution.id == execution_id,
                    TaskExecution.locked_by == worker_id,
                )
                .values(**values)
            )
            await session.commit()
    ...
    async def complete(
        self, execution_id, worker_id: str, result: dict | None = None
    ) -> bool:
        return await self._finalize(
            execution_id, worker_id, ExecutionStatus.succeeded.value, result=result
        )
        ...
        async with self._database.session_factory() as session:
            ...
            result = await session.execute(
                ...
                .where(
                    TaskExecution.id == execution_id,
                    TaskExecution.locked_by == worker_id,
                    TaskExecution.status == ExecutionStatus.running.value,
                )
                .values(**values)
            )
            await session.commit()
    ...
    async def reclaim_expired(self, grace_seconds: int = 0) -> int:
        ...
        expired = (
            TaskExecution.status == ExecutionStatus.running.value,
            TaskExecution.locked_until < cutoff,
        )
        ...
        async with self._database.session_factory() as session:
            failed = await session.execute(
                update(TaskExecution)
                .where(
                    *expired, TaskExecution.attempt_count >= TaskExecution.max_attempts
                )
                .values(
                    status=ExecutionStatus.failed.value,
                    locked_by=None,
