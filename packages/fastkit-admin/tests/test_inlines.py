...
class OrderLine(PrimaryKeyMixin, Base):
    ...
    order_id: Mapped[int] = mapped_column(
        BigInteger,
        ForeignKey("inline_orders.id", ondelete="CASCADE"),
        nullable=False,
        index=True,
    )
...
async def test_create_persists_inline_children(session):
    ...
    order = await admin.create(
        session, {"name": "First", "lines": [{"sku": "A"}, {"sku": "B"}]}, "en"
    )
...
async def test_inline_values_serializes_existing_children(session):
    admin = OrderAdmin()
    order = await admin.create(
        session, {"name": "First", "lines": [{"sku": "A"}]}, "en"
    )
    ...
    assert values == {
        "lines": [{"id": str(child_id), "sku": "A", "note": ""}],
        "tickets": [],
    }
...
async def test_update_diffs_inline_children_by_id(session):
    ...
    await admin.update(
        session,
        order.id,
        {"name": "First", "lines": [{"id": str(kept_id), "sku": "A2"}, {"sku": "C"}]},
        "en",
    )
...
async def test_inline_validation_errors_carry_the_exact_row_path(session):
    ...
    with pytest.raises(ValidationError) as exc:
        await admin.create(
            session,
            {"name": "First", "lines": [{"sku": "A"}, {"sku": ""}, {"sku": ""}]},
            "en",
        )
...
async def test_inline_supports_a_non_default_pk_field(session):
    admin = OrderAdmin()
    order = await admin.create(
        session, {"name": "First", "tickets": [{"label": "T1"}]}, "en"
    )
    ...
    await admin.update(
        session,
        order.id,
        {"name": "First", "tickets": [{"id": ref, "label": "T1-edited"}]},
        "en",
    )
