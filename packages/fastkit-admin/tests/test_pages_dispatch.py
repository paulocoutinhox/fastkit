...
_USER = SimpleNamespace(
    id="u1", display_name="Root", email="root@x.com", profile=None, timezone="UTC"
)
...
async def _report_data(name, session, locale, params, check):
    ...
    return {
        "title": "Sales",
        "columns": [{"key": "c", "label": "C", "align": "left"}],
        "rows": [{"c": "x"}],
        "formats": ["csv"],
        "filters": [],
    }
...
def _pages(
    ...
    return PagesDeps(
        AdminRenderer(), site, deps, config, None, None, report_data, profile_data
    )
...
async def test_dispatch_dashboard(site):
    response = await dispatch_screen(
        _pages(site), "", QueryParams(""), _USER, None, "en"
    )
...
async def test_dispatch_list_and_fragment(session, product_model, site):
    ...
    full = await dispatch_screen(
        pages, "products", QueryParams(""), _USER, session, "en"
    )
    assert 'data-testid="grid"' in _body(full)
    assert 'data-testid="sidebar"' in _body(full)

    fragment = await dispatch_screen(
        pages, "products", QueryParams("_fragment=table"), _USER, session, "en"
    )
...
async def test_dispatch_form_create_and_edit(session, product_model, site):
    ...
    create = await dispatch_screen(
        pages, "products/new", QueryParams(""), _USER, session, "en"
    )
    assert 'data-testid="form"' in _body(create)
    assert 'data-testid="field-name"' in _body(create)

    edit = await dispatch_screen(
        pages, "products/1/edit", QueryParams(""), _USER, session, "en"
    )
    assert 'data-record-id="1"' in _body(edit)


async def test_dispatch_form_fragment_renders_related_widget(
    session, product_model, tag_model
):
    ...
    fragment = await dispatch_screen(
        _pages(local), "things/new", QueryParams("_fragment=form"), _USER, session, "en"
    )
    body = _body(fragment)
...
async def test_dispatch_detail(session, product_model, site):
    ...
    response = await dispatch_screen(
        _pages(site), "products/1", QueryParams(""), _USER, session, "en"
    )
...
async def test_dispatch_report_full_and_fragment(site):
    ...
    full = await dispatch_screen(
        pages, "reports/sales", QueryParams(""), _USER, None, "en"
    )
    assert 'data-testid="report-table"' in _body(full)
    assert 'data-testid="sidebar"' in _body(full)

    fragment = await dispatch_screen(
        pages, "reports/sales", QueryParams("_fragment=table"), _USER, None, "en"
    )
...
async def test_dispatch_report_without_provider_is_not_found(site):
    with pytest.raises(NotFoundError):
        await dispatch_screen(
            _pages(site, report_data=None),
            "reports/sales",
            QueryParams(""),
            _USER,
            None,
...
async def test_dispatch_profile(site):
    response = await dispatch_screen(
        _pages(site), "profile", QueryParams(""), _USER, None, "en"
    )
...
async def test_dispatch_profile_without_provider(site):
    response = await dispatch_screen(
        _pages(site, profile_data=None), "profile", QueryParams(""), _USER, None, "en"
    )
...
async def test_dispatch_notfound(site):
    with pytest.raises(NotFoundError):
        await dispatch_screen(
            _pages(site), "a/b/c/d", QueryParams(""), _USER, None, "en"
        )
...
async def test_dispatch_enforces_permissions(session, product_model, site):
    ...
    response = await dispatch_screen(
        _pages(site, authorize=deny), "products", QueryParams(""), _USER, session, "en"
    )
...
async def test_dispatch_translates_navigation(session, product_model, site):
    ...
    response = await dispatch_screen(
        _pages(site, translate=translate),
        "products",
        QueryParams(""),
        _USER,
        session,
...
async def test_render_screen_redirects_anonymous(site):
    ...
    deps = SimpleNamespace(
        get_locale=_locale, authorize=None, translate=None, translator=None
    )
    pages = PagesDeps(
        AdminRenderer(), site, deps, config, None, None, _report_data, _profile_data
    )
