...
async def list_screen(target, session, check, locale: str, path: str, params) -> dict:
    ...
    return list_context(
        target.grid_schema(flags), result, path, query.search, query.sort, query.filters
    )
...
async def _attach_related_flags(schema: dict, site, check) -> None:
    for group in schema["fieldsets"] + schema.get("inlines", []):
        for field in group["fields"]:
            ...
            field["related_flags"] = {
                "add": flags["can_create"],
                "edit": flags["can_update"],
                "delete": flags["can_delete"],
            }


async def form_screen(
    target, session, check, site, locale: str, path: str, record_id, mode: str
) -> dict:
...
async def shell_context(
    ...
    avatar = (
        await pages.avatar_url(file_id)
        if (pages.avatar_url is not None and file_id)
        else None
    )
    display_name = user.display_name or getattr(user, "email", None) or "User"

    return {
        "config": request_config(pages.config, pages.translator, locale),
...
async def _forbidden(
    pages: PagesDeps, user, locale: str, current: str | None = None
) -> HTMLResponse:
    ...
    return HTMLResponse(
        pages.renderer.render(
            "admin/error.html",
            **await shell_context(pages, user, locale, current),
            message=message,
        ),
        status_code=403,
    )
...
async def dispatch_screen(
    pages: PagesDeps, sub: str, params, user, session, locale: str
) -> HTMLResponse:
    ...
    if kind == "dashboard":
        ...
        return HTMLResponse(
            render(
                ...
                **await shell_context(pages, user, locale, current, crumb, title),
            )
        )

    if kind == "profile":
        data = (
            await pages.profile_data(user, locale)
            if pages.profile_data is not None
            else {}
        )
        crumb, title = build_header(kind, args, path, t)
        ...
        return HTMLResponse(
            render(
                "admin/profile.html",
                **await shell_context(pages, user, locale, current, crumb, title),
                **profile_context(data, path, api_path),
            )
        )
    ...
    if kind == "report":
        ...
        try:
            data = await pages.report_data(
                args["name"], session, locale, dict(params), check
            )
        except AuthorizationError:
            return await _forbidden(pages, user, locale, current)

        context = report_context(
            data, args["name"], path, api_path, screen_query(params)
        )

        if fragment == "table":
            return HTMLResponse(
                render("admin/partials/_report_table.html", t=t, **context)
            )
    ...
    try:
        if kind == "list":
            ...
            if fragment == "table":
                return HTMLResponse(
                    render("admin/partials/_table.html", t=t, **context)
                )
        ...
        if kind == "form":
            await check_permission(
                pages.deps.authorize,
                user,
                target,
                "create" if args["mode"] == "create" else "update",
            )
            context = await form_screen(
                target,
                session,
                check,
                pages.site,
        ...
        detail = await detail_screen(
            target, session, check, locale, path, args["record_id"]
        )
        crumb, title = build_header(
            kind, args, path, t, label=label, display=detail.get("display")
        )
...
async def render_screen(pages: PagesDeps, request: Request, sub: str, user, session):
    ...
    return await dispatch_screen(
        pages, sub, request.query_params, user, session, locale
    )


def build_admin_pages_router(
    renderer,
    site,
    deps,
    config: dict,
    ...
    pages = PagesDeps(
        ...
        deps,
        config,
        translator if translator is not None else deps.translator,
        avatar_url,
        report_data,
        profile_data,
    )
    ...
    async def sub_page(
        request: Request,
        sub: str,
        user=Depends(deps.get_optional_user),
