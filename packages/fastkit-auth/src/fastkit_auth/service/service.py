...
from fastkit_auth.service.result import LoginResult
...
class AuthService:
    ...
    async def login(
        ...
        password: str,
        requested_tenant_id: int | None = 0,
        ...
        normalized_identifier = self._accounts.normalize_identifier(
            identifier_type, identifier_value
        )
        await self._rate_limiter.hit(
            ip_address,
            requested_tenant_id,
            f"{identifier_type}:{normalized_identifier}",
        )

        await self._captcha.verify(captcha)

        candidates = await self._accounts.find_candidates(
            requested_tenant_id, identifier_type, identifier_value
        )
        ...
        matches = [
            ...
            and self._passwords.verify(candidate.password_hash, password)
        ]
        ...
        if len(matches) > 1:
            raise AuthenticationError(
                AMBIGUOUS_IDENTITY, message="identifier matches more than one account"
            )

        user = matches[0]
        self._assert_can_authenticate(user)

        effective_tenant_id = resolve_effective_tenant(
            to_api(user.tenant_id), requested_tenant_id
        )
        record, raw_token = await self._sessions.create(
            user.id, to_api(user.tenant_id), effective_tenant_id, ip_address, user_agent
        )

        await self._on_success(user, password)
        token = self._tokens.issue(
            str(user.id), to_api(user.tenant_id), effective_tenant_id, str(record.id)
        )
    ...
    def _assert_can_authenticate(self, user: User) -> None:
        ...
        if (
            ...
            and ensure_aware(user.locked_until) > self._clock()
        ):
            raise AuthenticationError(
                ACCOUNT_LOCKED, message="account is temporarily locked"
            )
            ...
            for candidate in candidates:
                ...
                await session.execute(
                    ...
                    .where(
                        ...
                        User.failed_login_count >= self._max_failed,
                    )
    ...
    async def _on_success(self, user: User, password: str) -> None:
        ...
        rehashed = (
            self._passwords.rehash(password)
