...
from fastkit_files.images import ImageInfo, enforce_pixels, inspect, process_variant
...
def test_process_variant_modes(image_factory, mode):
    spec = ImageVariantSpec(
        name="v", width=64, height=64, mode=mode, format="webp", quality=80
    )
...
def test_process_variant_max_width_and_height(image_factory):
    ...
    wide = process_variant(
        data, ImageVariantSpec(name="w", width=100, mode="max_width", format="png")
    )
    assert wide.width == 100

    tall = process_variant(
        data, ImageVariantSpec(name="h", height=50, mode="max_height", format="png")
    )
...
def test_process_variant_max_width_no_upscale(image_factory):
    ...
    result = process_variant(
        data, ImageVariantSpec(name="w", width=200, mode="max_width", format="png")
    )
...
def test_process_variant_jpeg_converts_mode(image_factory):
    variant = process_variant(
        image_factory(fmt="PNG"),
        ImageVariantSpec(name="j", width=32, height=32, mode="cover", format="jpeg"),
    )
...
def test_process_variant_unknown_mode(image_factory):
    with pytest.raises(FastKitError, match="unknown resize mode"):
        process_variant(
            image_factory(),
            ImageVariantSpec(name="x", width=10, height=10, mode="warp", format="png"),
        )
...
def test_aware_helper():
    ...
    from fastkit_files.service import _aware
