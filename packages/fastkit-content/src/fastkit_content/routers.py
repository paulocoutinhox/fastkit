...
def build_content_router(runtime, security, publish_permission: str = "content.publish", tenant_id: int = 0) -> APIRouter:
    ...
    async def languages(user=Depends(security.get_current_user)):
        ...
        return success_envelope(data=[{"code": language.code, "name": language.name} for language in active])

    @router.get("/content/{content_id}/translations")
    async def get_translations(content_id: int, user=Depends(security.get_current_user)):
        await _require(user)

        return success_envelope(data={"translations": await content_service.translations_by_content_id(content_id)})

    @router.put("/content/{content_id}/translations")
    async def set_translations(content_id: int, payload: TranslationsPayload, user=Depends(security.get_current_user)):
        await _require(user)

        codes = {language.code: language.id for language in await language_service.list_active()}
        ...
        for entry in payload.translations:
            ...
            await content_service.set_translation(content_id, language_id, title=entry.title, summary=entry.summary, body=entry.body)

        return success_envelope(message=build_message("content.translations_updated", "Content translations updated."))

    @router.get("/content-by-key/{key}")
    async def read_by_key(key: str, language: str | None = None, user=Depends(security.get_current_user)):
        await _require(user)
