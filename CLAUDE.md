...
**E uma trava cobra a metade que se perde primeiro.** Um comentário que voltou para o outro idioma é
justamente o pedaço que quem lê de fora deste repositório não acompanha, então a varredura percorre todo
comentário e toda docstring do código que roda atrás de palavra marcada, e uma que um dia precise das
marcas é nomeada com o motivo. Ela prova que ainda reconhece uma sobre uma frase escrita dentro dela.

## O que está escrito aqui

| Seção | O que ela decide |
| --- | --- |
| [Como uma tarefa é feita aqui](#como-uma-tarefa-é-feita-aqui) | o que se faz por padrão, o que nunca se cria sem pedido, e onde a barra está |
| [Comandos](#comandos) | tudo pelo `make`, e o que cada receita lê |
| [Stack](#stack) | de que o projeto é feito, e onde cada afirmação é provada |
| [Estrutura do projeto](#estrutura-do-projeto) | a árvore, as três portas de entrada da raiz e as camadas do backend |
| [Padrão de código](#padrão-de-código) | formatação, comentário, a ordem de um arquivo e a forma de um método |
| [Nomenclatura e organização de arquivos](#nomenclatura-e-organização-de-arquivos) | onde uma classe nova vai, e como cada coisa se chama |
| [Como adicionar um recurso novo](#como-adicionar-um-recurso-novo) | os sete passos, o `CrudService` e o que as travas cobram |
| [Banco de dados](#banco-de-dados) | o nível de isolamento, a corrida entre duas escritas, o esquema sem migration e os índices |
| [Autenticação e permissão](#autenticação-e-permissão) | quem alcança o quê, a conta, a senha, e as nove perguntas que toda API nova responde |
| [Contratos da API](#contratos-da-api) | a forma de uma resposta, a busca, o idioma e o que o site e o aplicativo compartilham |
| [O site](#o-site) | as páginas, o formulário, os cookies, os temas, o dinheiro, a data e os assets |
| [Um provedor se escreve de uma forma só](#um-provedor-se-escreve-de-uma-forma-só) | o contrato dos quatro, e quando um erro é pego |
| [Captcha](#captcha) | os três provedores, e onde um desafio é pedido |
| [Assinaturas e entrega](#assinaturas-e-entrega) | plano, direito, benefício, ciclo e o que uma assinatura leva embora |
| [O comércio](#o-comércio) | o produto, a compra e a carteira |
| [O que um gateway de pagamento reporta](#o-que-um-gateway-de-pagamento-reporta) | o webhook, a normalização, e o RevenueCat e o Stripe campo por campo |
| [Jobs agendados](#jobs-agendados) | o que roda sozinho, a retenção, e por que uma passada acontece uma vez |
| [Storage](#storage) | as duas portas do bucket, a regra de cada finalidade e quando um arquivo sai |
| [Configuração](#configuração) | um arquivo por ambiente, o nome do produto e a tabela de limites |
| [O admin](#o-admin) | o painel dirigido por definição, os campos, as telas e a conta de quem opera |
| [Publicar](#publicar) | a imagem, o proxy e rodar mais de uma cópia |
| [Nada em produção sem perguntar antes](#nada-em-produção-sem-perguntar-antes-é-regra-não-preferência) | o que exige consulta explícita, sempre |
| [Testes](#testes) | a suíte, os três ensaios que ela não alcança, e como um verificador erra |
| [Commits](#commits) | a mensagem, e quando um commit acontece |
| [Armadilhas conhecidas](#armadilhas-conhecidas) | o sintoma de um lado, a causa e a saída do outro |
| [Antes de dar como pronto](#antes-de-dar-como-pronto) | a lista que fecha qualquer tarefa |
...
├── app.py               monta o app: log, cors, locale, erros, site, rotas, estáticos
...
**A raiz tem três pontos de entrada, e cada um responde a uma pergunta diferente.** Três nomes parecidos
ali é o que faz alguém abrir o errado, então eles se separam pelo que fazem:

| Quem | O que ele é | Quem o chama |
| --- | --- | --- |
| `app.py` | a aplicação montada, e o `app` que um servidor ASGI serve | o `uvicorn`, como `app:app` |
| `manage.py` | todo comando de linha de comando, num mapa sem queda por fora | uma pessoa, ou o `entrypoint.sh` antes de servir |
| `Makefile` | o atalho para os dois, e a única coisa que quem desenvolve digita | uma pessoa, e a CI |

**O módulo se chama `app` porque é isso que ele responde.** Um `main.py` diz em que ordem o Python
começou a ler e nada sobre o que há dentro, e ao lado de um `manage.py` e de um `Makefile` ele vira o
arquivo que ninguém sabe de cor qual é. O nome atravessa o `Makefile`, o `entrypoint.sh` e os guias, e
`tests/test_requirements.py` falha se um deles ficar para trás.
...
**E o que uma ferramenta escreve na máquina de quem desenvolve não é versionado.** A pasta `.claude` é
configuração daquele computador e daquela pessoa, então ela está no `.gitignore` junto do `dist`, do
`brand.css` que o build escreve e do DDL do `schema-diff`.
...
| `mailer.py` | como uma mensagem escrita sai deste processo, e a implementação de cada serviço que carrega e-mail |
...
chamá-la**, e nunca como ela está implementada por dentro. As 95 descrições de rota são publicadas em
...
- Sem ponto e vírgula separando sentenças numa mesma linha, **e isso vale para toda prosa deste projeto**
  — comentário, docstring, este arquivo, o `README.md` e os guias. Duas ideias coladas por um ponto e
  vírgula se leem como uma, e uma trava percorre os treze arquivos de prosa mais todo comentário e toda
  docstring do código que roda. Ela tira o que está em bloco ou em código antes de olhar, porque ali o
  ponto e vírgula é da linguagem, e prova que ainda reconhece um sobre uma frase escrita dentro dela.
...
- **A regra de um campo mora no tipo dele, e não no schema.** `Document`, `MobilePhone` e `Timezone` são tipos
...
> ao tratador de erro pelo `app.py`, em vez de buscada por ele.
...
pessoa abre. `tests/test_docs.py` percorre os 178 caminhos da API **e as páginas do site**, e a
...
| Identidade com regra | um tipo anotado em `schemas/common.py` | `Document`, `MobilePhone`, `Timezone` |
...
| `tests/test_docs.py` | número escrito em prosa que deixou de bater com a superfície, **nos dois idiomas** e nos treze arquivos |
| `tests/test_docs.py` | janela de retenção e tempo de vida de cache que a prosa afirma e a configuração declara de outro tamanho |
...
| `tests/test_docs.py` | duas ideias coladas por um ponto e vírgula, na prosa e em todo comentário do código |
| `tests/test_docs.py` | comentário ou docstring do código que voltou para o outro idioma, e palavra isenta que ninguém escreve mais |
...
| `tests/test_docs.py` | link desta documentação que aponta para um arquivo ou um título que não existe |
| `tests/test_docs.py` | seção deste arquivo que o índice não nomeia, e entrada do índice que nenhuma seção responde |
| `tests/test_docs.py` | helper que a tabela de helpers não nomeia, e nome na tabela que a pasta não tem |
...
| `tests/test_app.py` | a suíte armando menos do que o `app.py` arma, sem o motivo escrito |
...
| `tests/test_layout.py` | tarefa iniciada que ninguém observa, cujo erro morre dentro dela |
| `tests/test_layout.py` | chamada que sai desta máquina sem prazo, que espera pelo que a biblioteca escolheu |
| `tests/test_layout.py` | prazo que a tabela acima afirma e o módulo declara de outro tamanho |
...
| `tests/test_route_guards.py` | contar como guarda o que não guarda: a marca diz qual site responde, e a conta opcional lê quem estiver ali sem exigir ninguém |
...
| `tests/test_api_restrictions.py` | payload de edição que oferece escrever um nada numa coluna que a tabela declara com valor |
...
| `tests/test_sql_portability.py` | contador que duas requisições alcançam somado em Python, e exceção que sobrevive à linha que a pediu |
| `tests/test_sql_portability.py` | a própria varredura de contador deixando de reconhecer um, provada sobre uma linha escrita dentro dela |
...
| `tests/test_providers.py` | dois provedores do mesmo contrato escrevendo um método em formas diferentes, onde quem chama espera uma delas |
| `tests/test_providers.py` | número de contratos que a prosa afirma e a varredura não acha |
| `tests/services/test_gateway_shapes.py` | gateway cujo `authenticate` é corrotina, que nunca roda e deixa o webhook aberto, e cujo `read` não é |
...
| `tests/test_admin_contract.py` | campo que a tela da própria conta desenha e a API da conta não aceita |
...
| `tests/test_admin_contract.py` | rótulo de recurso que deixa uma palavra em minúsculo onde todos os outros a escrevem em maiúscula |
...
| `tests/services/test_crud.py` | dependente ou alvo de chave estrangeira que nenhum service serve, onde o mapa é lido por índice |
...
| `tests/routes/site/test_public.py` | página que escreve sem perguntar de onde o formulário veio |
| `webapps/admin/tests/components/fields.test.js` | campo do painel que fica vermelho e não diz que foi recusado |
| `webapps/admin/tests/components/edges.test.js` | temporizador que uma tela começou e que sobrevive à saída dela |
...
> **E ela é percorrida a cada chamada, nunca guardada.** `services_by_model()` lê `__subclasses__()`, que
> responde o que **já foi importado** — então guardar o resultado congela o mapa no instante da primeira
> chamada, e um service carregado depois fica de fora dele pelo resto do processo. Medido: perguntado
> antes de a aplicação estar montada o mapa responde **zero** modelos, e depois responde 31. Quem o lê é
> a exclusão, então o desfecho era uma exclusão que apaga a linha do filho e **deixa os arquivos dele no
> bucket, sem dizer nada**. Percorrer custa 9 microssegundos, e acontece uma vez por registro excluído.

> **E ele é lido por índice, e não por `get`.** Um dependente que nenhum service serve é uma declaração
> errada, e o certo é ela estourar onde está em vez de ser pulada em silêncio — que é como o furo acima
> se escondia. Uma trava percorre os dependentes declarados e os alvos de chave estrangeira e cobra que
> todos sejam servidos.
...
`Tenant`.** Em marca única ele não escreve tenant nenhum e tudo nasce no escopo nulo, e em muitas marcas
...
### Toda chamada que sai desta máquina carrega prazo

**Um servidor que aceita a conexão e depois não responde é o jeito de segurar um trabalhador sem
derrubar nada.** Não é a máquina que caiu — essa recusa na hora —, é a que atende e cala: sem prazo,
quem chamou espera o que a biblioteca escolheu, e esse é um número que ninguém deste lado decidiu.

| Quem é chamado | Quanto ele tem |
| --- | --- |
| o reCAPTCHA | 5s, porque um formulário está esperando |
| o código postal | 5s, pelo mesmo motivo |
| o RevenueCat, e o recolhimento do webhook | 10s |
| a reconciliação | 10s |
| o checkout | 15s, porque abrir uma sessão é o passo mais caro do fornecedor |
| **o servidor de e-mail** | 15s, e ele é o único que não é HTTP |

**A trava acha os módulos que falam com outra máquina**, lendo quem importa `httpx` ou `aiosmtplib`, e
cobra que cada cliente aberto e cada envio nomeie um prazo — nomear as chamadas à mão cobriria as que
alguém lembrou de nomear.

> **O que um prazo de e-mail custa, e por que ele é maior.** A passada da fila tem `timeout=110` e
> recolhe até 50 mensagens, então um servidor mudo com o padrão da biblioteca gasta a passada inteira em
> uma. O `claim` e o `reclaim_abandoned` devolvem a linha, então o desfecho continua seguro — o que se
> perde é a passada, e é isso que o prazo evita.
...
**E editar não é uma forma de esvaziar o que a linha tem que ter.** O `as_optional` deixa **todo campo
ser omitido** e não mexe na anotação dele, então o que pode ser escrito como nada continua sendo só o que
a coluna aceita como nada. Alargar a anotação para `Optional[...]` em todos deixava um nulo atravessar
até a coluna, e o que o banco responde a isso é a recusa que ele guarda para uma **duplicata**: quem
limpasse o gênero da própria conta lia *registro duplicado* sobre algo que não duplicava nada. Uma trava
percorre todo payload de edição e cobra isso, pulando as seis colunas que o service preenche sozinho —
`tag`, `slug` e `code` são derivados do título quando ninguém digita um, e ela acha quais são lendo as
chamadas de `apply_slug` no fonte em vez de guardar uma lista.
...
- **A conta é criada com pelo menos uma de quatro identidades:** username, e-mail, documento ou celular.
  Quem valida é `UserService.ensure_identity`, no cadastro e na edição.
- **O campo do documento é genérico de propósito, e a regra que o confere é a do CPF.** O nome diz o que
  a coluna guarda — um documento — e `is_valid_cpf` diz qual regra está aplicada hoje, porque quem
  trocá-la por outra precisa ler no nome o que está trocando. Esse é o mesmo desenho do telefone e do
  código postal: o campo é de todo mundo, e o país é que diz como ele se escreve.
...
primeira que ela tiver entre e-mail, username, documento e celular — **nessa ordem**, que é a que uma pessoa
...
Uma cascata, não um campo: `nickname`, depois `first_name` + `last_name`, depois `username`, depois **o
que vem antes do arroba** do `email`, e por último `#<id>`.
...
**O que identifica a pessoa nunca é o nome pelo qual ela é chamada.** O celular e o documento ficam de
fora da cascata, e o endereço entra pela metade: esse nome é desenhado no topo do painel, no card da
conta e em toda opção de lookup que nomeia um usuário — telas que quem opera o produto lê, e não só o
dono da linha. Uma conta que não tem nenhum dos quatro é chamada pelo `#<id>`, que não diz nada sobre
ninguém.
...
**O epoch é subido pelo banco, num `UPDATE`, e não por este lado** — é a mesma regra que a contagem de
senha errada segue, pelo mesmo motivo. Duas trocas que se cruzam leem o mesmo número e escrevem o mesmo
número, e aí as duas cunham token com ele e **as duas sessões continuam valendo**, que é exatamente o
contrário do que o campo existe para fazer. O cenário não é hipotético: é o da recuperação de senha, que
é quando alguém está tentando tirar outro de dentro da conta. A conta é relida depois da escrita, porque
as duas rotas cunham o token a partir dela.
...
| `session_epoch` | dizer quais sessões ainda valem | a cada troca de senha, **subido pelo banco** |
...
### O endereço responde antes de a conta responder

**A chave `confirm_sign_up` decide, e é uma linha de configuração.** Com ele ligado — que é o padrão — uma conta
criada pelo cadastro nasce `pending`, recebe uma mensagem no endereço que ela nomeou, e só passa a
`active` quando aquele endereço responde. Sem ele, o cadastro é o que sempre foi: a conta nasce aberta e
a sessão começa ali.

**Quem segura a conta fechada é o `status`, e não um campo novo.** O `ensure_usable` já recusava
`pending` antes de qualquer rota, com `error.account-pending` — a confirmação não inventou um estado,
ela passou a usar o que existia.

| Superfície | Ligado | Desligado |
| --- | --- | --- |
| `POST /api/signup` | 201 com `token` **nulo**, e a conta `pending` | 201 com o token, como sempre |
| o cadastro do site | volta para o login dizendo onde olhar, e **não** abre sessão | entra direto |
| `POST /api/account/confirmation/{token}` | abre a conta e responde o token | o token não existe, então ela responde 422 |
| `POST /api/account/confirmation` | escreve de novo ao endereço | não há conta esperando, e a resposta é a mesma |

**Um token de nada é o que o cadastro responde quando o endereço ainda tem que falar.** O `SignUpResponse`
carrega `token: str | None`, e é uma forma só: uma rota que respondesse dois schemas conforme a
configuração deixaria quem integra escrevendo dois caminhos de leitura.

**Uma conta sem endereço não tem o que provar.** As quatro identidades são intercambiáveis, então quem se
cadastra por username ou celular nasce `active` mesmo com a confirmação ligada — não há para onde
escrever, e segurar a conta seria trancá-la para sempre.

**E isso é do cadastro, e não de criar uma conta.** Quem passa pela confirmação é o
`auth_service.register`, que é por onde uma pessoa se cadastra sozinha. Um administrador cadastrando
alguém no painel e o `make seed` chamam o `user_service.create`, cuja coluna nasce `active` — senão o
ambiente local nasceria sem conta com que entrar, e o operador criaria contas que ele mesmo teria que
destravar.

**O token é sorteado junto com a janela, num `UPDATE` condicionado.** É a mesma forma da recuperação de
senha e do convite da newsletter, e pelo mesmo motivo: a rota que pede de novo é **aberta**, então sem a
janela um formulário escreveria à caixa de quem nunca pediu nada tão rápido quanto conseguisse postar.
Sortear o token junto é o que faz **só a última mensagem** abrir a conta — duas válidas na mesma caixa é
uma que ninguém sabe qual é.

**E pedir de novo responde igual para qualquer login.** Um que ninguém tem, um que já confirmou e um que
está esperando respondem os mesmos 204 — senão a rota vira uma forma de perguntar quem tem conta aqui, e
quem ainda não confirmou a dele.

**E na máquina de quem desenvolve a carta é impressa, e não mandada.** O `dev` declara o mailer
`console`, e nada é discado dentro de uma requisição de qualquer jeito: a mensagem entra na fila e o
`send_pending_emails` a recolhe a cada dois minutos. Cadastrar-se local é esperar aquela passada e ler o
link no log do servidor — quem não souber disso lê a feature como quebrada, que é o pior desfecho de uma
que funciona.

**Confirmar apaga o token junto com o estado**, na mesma escrita condicionada a a conta ainda estar
`pending`. Sem isso o mesmo link abriria de novo uma conta que já está aberta, e dois cliques no e-mail
não são duas confirmações.
...
O que é sobrescrito, todos com valor **sorteado**: `username`, `email`, `document`, `mobile_phone`,
...
Todo endpoint fica sob `/api/<modulo>`. Hoje são 178 caminhos em 40 grupos, sobre 35 tabelas.
...
| `search_fields` | identificador | qualquer pedaço, com `LIKE` | `slug`, `document`, `reference`, `email` |
...
>
> **E ela percorre a árvore inteira de services, e não um nível dela.** Com um `__subclasses__()` só,
> `content`, `gallery` e `plan` ficavam de fora — os três herdam de uma base própria —, e eram
> justamente três das tabelas mais buscadas conferidas por ninguém. A trava afirma quantos services
> lendo prosa ela achou antes de afirmar que todos estão cobertos.
...
| **confirmar o endereço do cadastro** | `/account/confirm/{token}`, e `/account/confirmation` para pedir de novo | `POST /api/account/confirmation/{token}` e `POST /api/account/confirmation` |
...
**Quem carrega a mensagem é um contrato, e o service não conhece fornecedor nenhum.** O
`helpers/mailer.py` é o quinto contrato do projeto — `Mailer` com um `send`, `PROVIDERS[EmailProvider]`
indexado, e hoje o `smtp` e o `console` como as duas implementações. Acrescentar a API do SES, do
Postmark ou de quem for é escrever uma classe e um valor no enum, e mais nada em lugar nenhum.

| A peça | O que ela é |
| --- | --- |
| `Letter` | a mensagem em partes — destinatário, assunto, html e o `reply_to` —, e nunca um envelope já selado |
| `Mailer.send(config, letter)` | o contrato, que recebe o `EmailSettings` **daquele tenant** junto da carta |
| `AddressRefused` | o jeito daquele fornecedor de dizer *não há ninguém aí*, traduzido para o único conceito que o resto do sistema lê |

> **O contrato recebe as partes porque um envelope é o formato de um fornecedor só.** Uma API REST quer
> remetente, destinatário, assunto e html, então montar o MIME e depois desmontá-lo seria escrever a
> gambiarra dentro do contrato. Quem monta o envelope é o provedor de SMTP, porque ele é o único que
> precisa de um.

> **E a supressão de endereço depende dessa tradução.** A pergunta *este endereço morreu* era respondida
> **dentro do service**, lendo uma exceção do aiosmtplib para decidir se uma linha de `SuppressedAddress`
> nasce. Um provedor que fale HTTP diz a mesma coisa com um status e um corpo, e não teria como responder
> ali. Agora cada provedor levanta `AddressRefused`, e o service pergunta uma coisa só.
...
/account/confirm/{token}   o endereço dizendo que é o endereço, e a única de `/account/` que abre uma sessão em vez de exigir uma
/account/confirmation      pedir a carta de novo, para quem nunca a recebeu e por isso não consegue entrar
...
| 2 | o cookie de idioma | é o único lugar que sobra para quem não tem conta |
...
**E onde o cadastro pede que o endereço responda primeiro, o destino viaja dentro da carta.** A espera
pela confirmação é o único trecho da jornada que o navegador não atravessa — ela pode terminar horas
depois e em outro aparelho —, então o link que o e-mail carrega leva o `next` e a rota que o abre passa
o valor pelo mesmo `landing` de sempre. Ele é confinado duas vezes: ao ser escrito na carta e ao ser
lido de volta. Pedir a carta de novo não carrega destino nenhum, porque aí a jornada começou na página
de confirmação e não naquela que pediu sessão.
...
**E virar a página troca o número da página e mais nada.** O link sai de
`request.url.include_query_params(page=…)`, e não de um `?page=N` escrito à mão — este último **apaga o
resto do endereço**, então o dia em que uma listagem paginada ganhar um filtro, a segunda página responde
a listagem inteira e quem estava lendo não tem como voltar ao recorte. Nenhuma das três páginas que
paginam hoje carrega outra coisa na query, e é justamente por isso que o defeito ficaria escondido até
alguém pagar por ele: a peça é de todo mundo.
...
**A escolha é uma preferência, e é guardada como o idioma.** O cookie `theme_cookie`, escrito por
...
deixa o aparelho decidir. **E ele obedece ao consentimento**, exatamente como o `language_cookie`.
...
**5 coisas neste projeto são um contrato com uma implementação por fornecedor**: o captcha, o
provedor de código postal, o gateway de pagamento, o storage e **quem carrega o e-mail**. O eixo que
importa é **quando um erro é pego**:
...
**E todo provedor é alcançado por um índice, nunca por queda.** `PROVIDERS[enum]` nos cinco, e o
provedor é escolhido por enum e não por um `Literal` — um `if` cujo último ramo entrega tudo que ninguém
classificou é um provedor novo virando outro em silêncio.

> **E a configuração é tipada pelo enum, e não por um `Literal` que repete os valores dele.** Um
> `Literal` ao lado de um enum com os mesmos valores são duas declarações do mesmo conjunto, e quem lia a
> configuração convertia uma na outra a cada chamada — no dia em que as duas divergem, a conversão levanta
> `ValueError` **dentro de uma requisição**. Os três provedores e o formato de imagem escrevem o enum, o
> mapa é indexado direto, e a validação do pydantic recusa o valor errado onde a configuração nasce.
>
> **E isso é o que põe a tabela debaixo da trava.** A varredura de conjuntos fechados só enxerga o que é
> chaveado por membro de enum, então uma tabela indexada por string crua fica de fora dela — o formato de
> imagem era a última assim, e passou de invisível a conferida sem escrever trava nenhuma. O que continua
> `Literal` é o que não indexa tabela: `store` e `scheme` são ramos, e um ramo não tem com o que divergir. Cada provedor de storage recebe o
`StorageSettings` e lê dele o que precisa, que é o que faz o mapa ser indexável sem um `if` para montar
cada um.

**E onde a configuração é de cada marca, ela chega na chamada e não na construção.** O storage é do
ambiente, então o provedor dele nasce com o `StorageSettings` na mão. O e-mail não é: `email_for` responde
o que **aquele tenant** declara, então `send(config, letter)` recebe o config junto da carta e o mapa
continua sendo um mapa de instâncias.

**Todo corpo que outra máquina respondeu é lido por `helpers.remote.body_of`**, nos cinco, e nenhum
`.json()` cru sobrou.
...
**E ele cobra que os provedores de um contrato escrevam cada método da mesma forma**, comparando-os
entre si em vez de ler uma lista: um esperado e o outro não é a diferença entre um valor e um objeto que
ninguém lê, e quem chama foi escrito para exatamente um dos dois. É o que pega um `issue` de captcha que
virou corrotina, um `walk` de storage que deixou de ser, e o `authenticate` de gateway, que é onde isso
abre a porta.

> **E um método que responde um fluxo diz isso no tipo.** O `walk` do storage é o único que não responde
> um valor, e sem o `AsyncIterator` no retorno o contrato não dizia nada sobre a única coisa dele que não
> se adivinha — quem escrevesse um provedor novo com um `def` comum só descobriria dentro do `async for`
> da varredura de órfãos.
...
> **Os três são idempotentes, e um tipo novo não é obrigado a ser — mas precisa saber que não é.** O
> jeito de escrever um que não é tem forma conhecida: **somar sobre o que está de pé em vez de sobre
> hoje**, que é o certo para uma data que se renova — somar sobre hoje joga fora o resto do mês a cada
> renovação — e é exatamente o que faz a segunda entrega deixar de ser um nada e virar uma segunda soma.
> Quem escrever um assim recebe a proteção da reivindicação da passada de recolhimento, e não da
> propriedade do próprio tratador. Foi um tipo desses, num produto construído a partir deste template,
> que mostrou que a passada não podia depender de todo tratador ser idempotente.
...
**E o `make seed` liga os dois lados**, com um `ExternalProduct` por plano semeado: o `external_id` é o
que o gateway reportaria, e o `plan_id` é o que ele vira aqui. Sem essa linha o ambiente local tem um
gateway que não resolve plano nenhum, e a tela de produtos externos nunca é vista com dado — que é o
mesmo motivo pelo qual todo teste que exercita o caminho de gateway cria uma.

**O que o seed não enche são as tabelas operacionais**, e isso é a regra e não um esquecimento: evento
reportado pelo cliente, inscrição de newsletter, fila de e-mail e registro do sistema nascem de alguém
**operar** o produto, e não de semeá-lo. Em marca única a lista de marcas também nasce vazia, porque ali
não há tenant nenhum a escrever.
...
**E ela recusa também o método escrito na forma errada, que é a que custa mais caro aqui.** Quem chama
`authenticate` **não espera por ele** — a prova é um `raise` e não um valor —, então um `authenticate`
escrito `async` devolve um objeto que ninguém lê: ele nunca roda, nunca levanta, e **toda chamada que
alguém fizer ao endereço do webhook passa como assinada**. A rota é aberta de propósito, porque a
credencial é a chave sorteada no caminho, então isso é a conferência de assinatura desligada para o
mundo inteiro. O `read` e o `state_from_query` são o contrário: são esperados, e escritos sem `async`
entregam um valor onde uma corrotina é aguardada.
...
O e-mail, o evento reportado pelo cliente, o aviso do gateway e **o recolhimento de concessões** fazem a
mesma coisa: um `UPDATE` condicionado ao estado ainda ser o de espera, um commit por item, e um
recolhimento do que ficou tomado por mais que `ABANDONED_AFTER`. Sem isso, duas passadas que se
sobreponham leem as mesmas linhas e processam as duas — e a sobreposição não é hipótese, porque um job
de 10 minutos tem `timeout=590`, e o `manage.py run-delivery` chama as mesmas funções por fora do
agendador.

**A da entrega é a que menos parece precisar, e é a que mais custou entender.** A passada principal já
está protegida, mas por outra coisa: ela **cria** a concessão, e a `grant_key` única resolve a corrida
entre duas que criam a mesma. O recolhimento trabalha uma linha **que já existe**, onde aquela chave não
protege nada. O que isso custava aqui não era pagar duas vezes — os três tipos de benefício são
idempotentes, o crédito pela `grant_key`, a posse por ser uma linha e o acesso por ser um estado ligado
— e sim:

| O que | Por quê |
| --- | --- |
| o orçamento de tentativas queimava em dobro | cada passada subia `attempts`, então o `give_up` fechava o ciclo antes das cinco que a configuração promete |
| a passada só era segura por coincidência | ela dependia de **todo** tratador ser idempotente, que é propriedade dos três tipos de hoje e não da passada — a reivindicação é o que a torna segura por construção, e o que poupa o quarto tipo de ter que ser |

**E o predicado é escrito uma vez.** O `takeable()` é lido pela consulta e pela reivindicação, senão os
dois se afastam e a passada lê uma coisa e toma outra.

**E com isso nenhum contador deste projeto é mais somado em Python.** O `attempts` da concessão era o
último, e ele era **isento** na trava dos contadores com o motivo escrito — um motivo que dizia,
literalmente, que perder uma soma custava *um give-up antecipado*. Era o defeito descrito como se fosse
uma escolha. Agora quem soma é o `UPDATE` da reivindicação, a isenção sumiu, e a varredura ficou sem
sujeito: ela prova que ainda reconhece um contador rodando sobre uma linha escrita dentro dela mesma, em
vez de depender de o produto guardar um.

> **E a passada responde o que ela trabalhou, e não o que ela leu.** O job registra quantos registros
> ele tocou, então devolver a leitura contaria como trabalho a linha que outra passada levou.

**A do gateway tem um segundo leitor, e é ele que torna a disputa comum:** a entrega ao vivo e a passada
de recolhimento chegam na **mesma linha**, porque o gateway reentrega justamente o que falhou. As duas
reivindicam antes de ler, e quem perde devolve a linha como ela está. O teto de tentativas fica na
passada, que o lê na própria consulta, senão um gateway que continua entregando seria recusado por ele.
...
**E o worker é uma tarefa que alguém lê.** `asyncio.create_task` guarda dentro da tarefa o que ela
levantou, então um worker que cai enquanto o processo serve não aparece em lugar nenhum: o site, o
painel e a API seguem respondendo, a sonda de prontidão responde 200, e **nenhum job roda mais naquele
nó** — o e-mail não sai, o ciclo não corre, a retenção não poda. Ele carrega um observador que registra
a queda em `error`, e o desligamento aguarda só o que ainda está de pé, porque o que parou sozinho já
foi lido. `tests/test_layout.py` recusa a próxima tarefa que ninguém observe.
...
**E onde esse nome tem que ser um identificador, ele é o `SLUG`**, que é o `NAME` passado pelo
`slugify` na linha de baixo. É dele que saem os **sete** cookies que este site escreve e o contêiner
descartável do `schema-diff`. **Isso é deste repositório ser um template:** quem parte dele troca o
`NAME` no primeiro dia, e sem a derivação continuaria escrevendo um cookie com o nome deste template no navegador de
**todo leitor do produto dele** — um nome que não é o dele, num valor que ele não tem como perceber
sem abrir as ferramentas do navegador.

| Onde | Como |
| --- | --- |
| os sete cookies | `session`, `flash`, `csrf`, `language`, `theme`, `visitor` e o do consentimento, todos `f"{SLUG}_…"` em `config/base.py` |
| o contêiner do `schema-diff` | `f"{SLUG}-schema"` |
| a política de cookies que o `make seed` escreve | ela nomeia os cookies lendo a configuração, senão o texto semeado conta ao leitor um nome que o site não escreve |

> **O nome do csrf mora com os outros seis.** Ele era o único escrito dentro de um helper, e um cookie
> declarado fora do lugar onde os outros são declarados é o que uma renomeação deixa para trás.

**E o painel não carrega o nome do produto em chave nenhuma.** As quatro coisas que ele guarda no
`localStorage` — `admin.token`, `admin.user`, `admin.locale` e `admin.theme` — nomeiam **a superfície**,
porque é isso que elas são. Derivá-las do `NAME` pediria o `slugify` escrito uma segunda vez em
JavaScript, e a mesma regra em duas linguagens é a que um dia diverge sem nada acusar — enquanto o
`localStorage` é de uma origem só, então não há com quem colidir. **Uma trava lê `config/base.py` e todo
módulo do código que roda**, cobra que o `SLUG` seja o `slugify` do `NAME`, que os sete cookies comecem
por ele, e falha se qualquer módulo voltar a soletrar um nome de cookie.
...
| Espera de um endereço entre duas confirmações de cadastro | 60 s | `confirm_sign_up_interval` |
...
| Opção | Efeito |
| --- | --- |
| `dependsOn` | um nome ou uma lista, e a consulta vai filtrada por ele com o campo fechado até o pai responder |
...
**E o rótulo de um recurso segue a mesma caixa, nos três catálogos e nas três chaves** — `menu`, `title`
e `singular`. Um menu é lido como uma coluna, então a única entrada que deixou uma palavra em minúsculo
lê como a que alguém esqueceu de terminar: *Produtos do Usuário* ao lado de *Produtos Externos*, e nunca
*Produtos do usuário*. O que fica em minúsculo é a palavra pequena de cada idioma — preposição e artigo —,
e uma trava percorre os 279 rótulos cobrando isso. Ela existe porque quatro deles ficaram fora do padrão
sem ninguém notar, e o quinto entrou junto da tela da própria conta.
...
**E a foto com o nome são o link para a própria conta.** Quem desenha os dois é o `AccountAvatar`, a mesma
peça que a tela de perfil usa, porque desenhar iniciais em dois lugares é a segunda cópia que um dia
discorda da primeira.

**Todo controle da direita tem a altura da foto**, que são 32 pixels, e o traço que os separa é de quem
divide a barra e não de quem é clicado — uma borda no próprio link é desenhada como um arco cortado ao
lado da imagem, porque o link é arredondado.

**Numa tela estreita o link sai inteiro, e quem carrega o caminho é a barra lateral.** O cabeçalho de um
celular não comporta os dois: medido em 375, o link come exatamente os 32 pixels que faltam para o título
da tela caber, e um título cortado em toda tela é pior do que a foto aparecer só a partir de `sm`. A
entrada do perfil fica no topo do menu, ao lado do painel inicial — são as duas que o menu desenha sem
perguntar permissão nenhuma, porque nenhuma das duas é recurso.

### A conta de quem opera o painel

**O que um operador edita de si mesmo não é um recurso**, e por isso `/profile` é uma tela escrita à mão,
ao lado do login, do painel inicial e do 404. O que ela escreve são os endereços que a própria conta já
responde por si:

| O que a tela faz | Por onde |
| --- | --- |
| o nome, o apelido, o gênero, o e-mail, o celular e o fuso | `PUT /api/account/me`, com os mesmos grupos de campo de qualquer formulário do painel |
| a foto | `POST` e `DELETE /api/account/avatar` |
| a senha | `POST /api/account/password` |

**E é por não ser um recurso que ela é alcançada por todo papel do painel.** Um editor edita a si mesmo
ali sem alcançar `users`, que é o recurso administrativo e chega na linha dos outros. A guarda do
roteador só cobra permissão de endereço que nomeia recurso, então não há nada a declarar aqui — e a rota
é declarada **antes** de `/:resource`, que casa com qualquer segmento.

**A foto é uma chamada e não duas.** A rota grava a imagem e já responde o `AccountSchema` inteiro, então
o painel nunca segura uma chave de storage da conta — é a mesma regra que vale para todo cliente.

**Trocar a senha troca o token deste aparelho e não esquece o que a conta alcança.** O `apply` do store
existe para **outra** conta chegando, e ele limpa as permissões de propósito: usá-lo aqui esvaziaria o
menu na frente de quem só trocou a própria senha. Quem troca o token da mesma conta é o `settle`.

**E o que a API respondeu é gravado no store.** Sem isso a foto e o nome do canto continuariam sendo os
do instantâneo que a entrada guardou, e mudariam só no próximo boot.

**Os campos dela são escritos à mão, então uma trava os confere.** Uma definição de recurso é lida pela
varredura que cobre os 31, e esta tela não é um recurso — sem a trava, um campo renomeado no schema
deixaria a tela mandando um nome que a API recusa, ou apontando a recusa para um campo que ela não
desenhou. A trava lê os construtores da própria tela contra os dois payloads que uma conta manda sobre
si, nas duas direções, e nomeia o `languageId` como o único que ela não desenha — o idioma do painel é
escolhido na barra de cima.

> **E o vocabulário de campo com que ela é lida sai do `resources/fields.js`**, e não de uma lista
> escrita dentro do teste: a lista à mão tinha esquecido o `timezone`, então todo campo de fuso do
> painel passava por fora das travas de contrato sem ninguém saber.
...
**E ela arma o que o `app.py` arma.** O que ela deixa de fora carrega o motivo escrito, e uma trava
...
> **E o `run_scoped` esvazia o pool na saída e não na entrada, o que foi medido e não julgado.** Contra um
> MySQL descartável, duas passadas em sequência respondem as duas — porque a primeira já esvaziou —, e o
> que quebra é um `asyncio.run` aberto **por fora** dele, que deixa uma conexão presa a um laço morto.
> Esvaziar também na entrada seria uma segunda resposta para a pergunta que a trava do laço já responde.
...
| dezesseis pedidos da mesma confirmação de cadastro | **um e-mail enfileirado**, e sem a leitura do `rowcount` os mesmos dezesseis viram **dezesseis** |
| dezesseis passadas sobre a mesma concessão que falhou | **uma tomou**, e a linha ficou com uma tentativa a mais — sem a leitura do `rowcount`, as dezesseis tomam |
...
| dezesseis leitores sobre o mesmo aviso de gateway | **um reivindicou**, e a linha ficou com uma tentativa |
| dezesseis trocas de senha simultâneas na mesma conta | **epoch 16**, uma por troca, e nenhuma sessão sobrevive ao que devia encerrá-la |
...
>
> **E de novo, na troca de senha:** subindo o epoch a partir do valor em memória em vez de mandar o banco
> subi-lo, as mesmas dezesseis trocas terminam em **epoch 7** — nove incrementos perdidos, que são nove
> sessões que uma troca de senha devia ter encerrado e não encerrou.
...
### O ensaio que só um navegador de verdade acusa

**Um terceiro ensaio existe e ele é o do navegador.** A suíte desenha HTML e o jsdom monta componentes,
e nenhum dos dois executa a página como um navegador executa: erro de JavaScript, requisição que falha
depois de a tela montar e largura que vaza de lado não aparecem em nenhum dos dois. Ele é ferramenta, e
por isso não mora no repositório — sobe o servidor numa porta livre, dirige, e some.

**O que ele mede, e o que foi medido:**

| O que se dirigiu | O que aconteceu |
| --- | --- |
| 31 recursos do painel, listagem e formulário novo | **62 telas sem um erro** de console, de página ou de rede |
| visualização e edição de um registro real de cada recurso | **52 telas**, idem |
| as páginas públicas, uma galeria seguida por link | **14 páginas**, idem |
| tudo isso a 375 de largura | **103 telas**, e nenhuma rola de lado |
| o site inteiro **sem JavaScript** | as dez páginas desenham, o idioma vira `en` → `pt`, o tema vira, e o aviso de cookies é respondido — tudo como formulário puro |

> **E ele prova o instrumento antes de afirmar qualquer coisa.** Um endereço que não existe e um erro
> lançado de propósito têm que aparecer nas três escutas, e uma caixa de 2000px tem que fazer a página
> rolar de lado. Sem isso, "nenhuma reclamação" é o que uma sonda quebrada também responde.

> **E ele erra do mesmo jeito que um teste erra.** Duas vezes ele mediu outra coisa: uma porta já
> ocupada deixou um servidor **antigo** respondendo, e um `make seed` no meio trocou o identificador da
> conta e derrubou a sessão. Nos dois casos o sintoma era idêntico ao de um defeito do produto.
...
| **toda varredura tem piso**: ela afirma quantos itens encontrou antes de afirmar que nenhum estava errado | uma expressão que parou de casar deixa a trava passando para sempre sem conferir nada — e o piso é da **varredura**, não de uma lista ao lado dela |
| **uma varredura que anda na árvore de classes anda nela inteira** | `__subclasses__()` responde um nível, e a trava de `FULLTEXT` deixava de conferir `content`, `gallery` e `plan` por isso |
| **uma varredura que procura um nome procura toda grafia dele**, e anda na mesma árvore de pastas que o resto | ler só `from helpers.db import …` é não enxergar `from helpers import db`, e um `*/*.py` é não enxergar `routes/site/` — a trava do laço tinha as duas cegueiras |
| **uma varredura que procura um comando lê código, e nunca a prosa em volta dele** | a trava dos comandos lia `make ([a-z]\w*)` no texto inteiro, então a frase inglesa *make that worth having* virava uma receita que o `Makefile` não tem |
| **um teste que dirige um estado que o produto recusa criar está medindo outra coisa** | um teste de corrida escrevia à mão um benefício de crédito sem moeda, e o service recusa exatamente isso — o que ele exercitava era um caminho que não existe |
...
| **uma trava que nomeia o que ela confere cobre o que alguém lembrou de nomear** | achar no fonte cobre o próximo sem ninguém precisar lembrar — o vocabulário de campo do painel, escrito à mão dentro do teste, tinha esquecido o `timezone` |
...
| 500 ao gravar duas vezes o mesmo vínculo | um `db.commit()` direto, onde quem traduz a colisão é o `helpers.db.commit` |
...
| `MissingGreenlet` ao serializar | uma relação que ninguém carregou, e quem a carrega é o `relations` |
| `MissingGreenlet` depois de um conflito de unicidade | um `rollback()` expira **todos** os objetos da sessão, e o que desfaz só a inserção é o `begin_nested()` |
...
| Linha compartilhada some do filtro | um `IN (id, NULL)` não casa com nulo, e quem casa é o `reaches_tenant` |
| Dois usuários globais com o mesmo e-mail | um `UNIQUE(tenant_id, email)` não vê nulo, então a unicidade é sobre `COALESCE(tenant_id, 0)` |
...
| Validação recebe `None` num campo com default | o `exclude_unset=True` descarta o que não veio, e quem lê o valor de verdade é o `declared()` |
...
| Uma concessão que falhou gasta o orçamento de tentativas em dobro | o recolhimento toma a linha antes de trabalhá-la, e só trabalha o que ganhou |
| Um mapa de services responde menos do que existe | ele é percorrido a cada chamada, porque guardá-lo o congela na ordem de import |
| Nenhum job roda mais num nó e a aplicação responde normalmente | o worker caiu e a tarefa engoliu o erro: ela carrega um observador que registra a queda |
| Trocar a senha não derruba a sessão que a outra troca acabou de abrir | o epoch é subido pelo banco, senão as duas escrevem o mesmo número |
| Uma reentrega do gateway é lida enquanto a passada está dentro daquela linha | as duas reivindicam antes de ler, e quem perde devolve a linha como ela está |
| Uma rota aberta passa na trava sem ninguém ter escrito que ela é aberta | nem a marca nem a conta opcional são guarda: quem alcança é declarado no `OPEN`, com o motivo |
...
| Virar a página de uma listagem filtrada responde a listagem inteira | o link escreve o número da página sobre o endereço que já estava ali |
...
| Um item do menu lê como o que alguém esqueceu de terminar | o rótulo de um recurso carrega a caixa que todos os outros carregam, nas três chaves |
...
| Um erro sobre a listagem aparece numa tela que já é outra | o debounce da busca é cancelado quando a tela sai |
...
| Limpar um campo no painel responde "registro duplicado" | a coluna não aceita nada, e o payload de edição parou de oferecer isso |
| A tela do próprio perfil responde a listagem de um recurso | a rota `/:resource` casa com qualquer segmento, então `/profile` é declarada antes dela |
| Trocar a própria senha esvazia o menu do painel | o `apply` é para outra conta chegando, e a troca do token da mesma conta é o `settle` |
| A foto e o nome do canto continuam os de ontem depois de editados | o que a API respondeu é gravado no store, que é o que a barra desenha |
| O título da tela aparece cortado num celular | a barra tem largura para uma coisa a mais, e o que sai dela é o link da foto, que o menu carrega |
| Uma borda vira um arco cortado ao lado da foto | o traço é de quem divide a barra, e o arredondamento é de quem é clicado |
...
| Um produto novo escreve nos leitores dele um cookie com o nome deste template | todo cookie é `f"{SLUG}_…"`, e o `SLUG` é o `NAME` reduzido a identificador |
...
| Um servidor que atende e cala segura um trabalhador | toda chamada que sai desta máquina carrega prazo, e a trava acha quem fala com outra |
...
| O cadastro responde 201 e o cliente não recebe sessão nenhuma | com `confirm_sign_up`, o token vem nulo até o endereço responder |
| Cadastrar-se na máquina local parece não mandar e-mail nenhum | o `dev` imprime a carta no log, e a fila a recolhe a cada dois minutos |
| Quem se cadastrou nunca consegue entrar e não há como reenviar | a rota aberta de reenvio existe, com janela, e a página dela está linkada no login |
| Duas mensagens de confirmação na caixa e só uma abre a conta | o token é sorteado junto com a janela, então a última é a que vale |
| O `make seed` deixa o ambiente sem conta com que entrar | quem confirma é o cadastro, e o `user_service.create` nasce `active` |
| O celular ou o documento de alguém aparece como nome numa tela do painel | o nome pelo qual uma conta é chamada só cai no que é seguro mostrar, e de um endereço só o que vem antes do arroba |
...
| Um aviso do mapeador aparece na suíte e nenhum filtro o transforma em erro | ele é emitido do outro lado da ponte de greenlet, onde o contexto por teste do pytest não alcança — o que acha um é ler o resumo da suíte |
...
| Todo webhook passa na conferência de assinatura e ninguém percebe | um `authenticate` escrito `async` não é esperado por quem o chama: ele não roda e não levanta |
| Um provedor de storage novo quebra dentro da varredura de órfãos | o `walk` responde um fluxo, e o contrato diz isso no tipo de retorno |
| Um provedor de storage novo vira S3 sem ninguém pedir | ele é indexado por enum, e não alcançado pelo último `if` |
| Trocar o serviço que manda e-mail pede mexer no service da fila | quem carrega a mensagem é um contrato, e o service entrega um `Letter` |
| Um provedor de e-mail novo nunca suprime um endereço morto | a recusa do destinatário é traduzida pelo provedor num `AddressRefused` |
| A suíte prova algo que a aplicação de verdade não faz | o `conftest` arma o que o `app.py` arma, e o que fica de fora carrega o motivo |
...
| Um número envelhece num guia e a trava não reclama | ela lia só este arquivo e o `README.md`, e agora lê os onze guias e as duas grafias |
| Um link da documentação leva a uma página ou a um título que não existe | a trava resolve todo link interno, arquivo e âncora |
