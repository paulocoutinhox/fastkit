...
LOGIN_UNIQUE_FIELDS = (("username", User.username, "error.username-already-used"), ("email", User.email, "error.email-already-used"), ("document", User.document, "error.document-already-used"), ("mobile_phone", User.mobile_phone, "error.mobile-phone-already-used"))
...
class UserService(CrudService):
    ...
    async def find_by_login(self, db: AsyncSession, login: str, tenant_id: int | None) -> User | None:
        ...
        if digits:
            conditions.extend([User.document == digits, User.mobile_phone == digits])
    ...
    async def find_by_confirmation_token(self, db: AsyncSession, token: str) -> User | None:
        return await db.scalar(select(User).where(User.confirmation_token == token))
