...
class WebhookService:
    ...
    async def ingest(self, db: AsyncSession, integration: Integration, call: InboundCall) -> WebhookEvent:
        ...
        # A redelivery of an event the retry pass is holding is the same row twice, and only whoever claims it reads it.
        if not await self.claim(db, record.id):
            return record

        return await self.run(db, integration, event, record)

    async def claim(self, db: AsyncSession, record_id: int) -> bool:
        """Takes an event for this reader, and answers whether it was the one that got it: a redelivery and the retry pass reach the same row."""
        # The ceiling on attempts is the pass reading its own queue, so a gateway still delivering is never refused by it.
        takeable = or_(WebhookEvent.status.in_((WebhookEventStatus.RECEIVED, WebhookEventStatus.FAILED)), and_(WebhookEvent.status == WebhookEventStatus.PROCESSING, WebhookEvent.updated_at < now() - ABANDONED_AFTER))
        statement = update(WebhookEvent).where(WebhookEvent.id == record_id, takeable).values(status=WebhookEventStatus.PROCESSING, attempts=WebhookEvent.attempts + 1)
