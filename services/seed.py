...
CONTENTS = [
    ...
    },
    {
        ...
        "language": "en",
        "category": "legal",
        "body": f"<h2>What this site keeps</h2><p>A cookie is a small value a site asks your browser to hold on to. This one keeps what it needs to answer you, and asks before keeping anything else.</p><h2>The necessary ones</h2><p><code>{settings.site.session_cookie}</code> is how the site knows you signed in. <code>{settings.site.csrf_cookie}</code> is how a form proves it was drawn here. <code>{settings.site.flash_cookie}</code> carries a notice from one page to the next, and <code>{settings.site.consent.cookie}</code> is the answer you gave on this page. None of them can be turned off, because without them the site cannot answer you at all.</p><h2>Preferences</h2><p><code>{settings.site.language_cookie}</code> keeps the language you chose, so you are not asked again on every page.</p><h2>Analytics and marketing</h2><p>This installation sets none. If it ever does, they are switched on by the choice you make on this page and never before it.</p><h2>Changing your mind</h2><p>Withdrawing is as easy as giving: open this page from the footer and choose again. Your answer is kept for six months, and you are asked again whenever this page changes.</p><p>Replace this text from the admin: it is a content record with the tag <code>cookies</code>.</p>",
    },
    {
        ...
        "language": "pt",
        "category": "legal",
        "body": f"<h2>O que este site guarda</h2><p>Um cookie é um valor pequeno que um site pede para o seu navegador guardar. Este guarda o que precisa para responder você, e pergunta antes de guardar qualquer outra coisa.</p><h2>Os necessários</h2><p><code>{settings.site.session_cookie}</code> é como o site sabe que você entrou. <code>{settings.site.csrf_cookie}</code> é como um formulário prova que foi desenhado aqui. <code>{settings.site.flash_cookie}</code> leva um aviso de uma página para a seguinte, e <code>{settings.site.consent.cookie}</code> é a resposta que você deu nesta página. Nenhum deles pode ser desligado, porque sem eles o site não tem como responder você.</p><h2>Preferências</h2><p><code>{settings.site.language_cookie}</code> guarda o idioma que você escolheu, para não perguntar de novo a cada página.</p><h2>Análise e publicidade</h2><p>Esta instalação não usa nenhum. Se um dia usar, eles são ligados pela escolha que você faz nesta página e nunca antes dela.</p><h2>Mudar de ideia</h2><p>Retirar é tão fácil quanto dar: abra esta página pelo rodapé e escolha de novo. Sua resposta é guardada por seis meses, e você é perguntado de novo sempre que esta página mudar.</p><p>Troque este texto pelo painel: ele é um registro de conteúdo com a tag <code>cookies</code>.</p>",
    },
    {
        "title": "Política de cookies",
        "tag": "cookies",
...
class SeedService:
    ...
    async def run(self, db: AsyncSession) -> dict:
        ...
        external = await self.seed_external_products(db, integrations, plans)
        ...
        return {
            ...
            "external products": len(external),
    ...
    async def seed_external_products(self, db: AsyncSession, integrations: dict[str, Integration], plans: list[Plan]) -> list[ExternalProduct]:
        """A gateway reports the product it sold, and this is the row that answers which plan that is."""
        mapped = []

        for code, integration in integrations.items():
            for plan in [plan for plan in plans if plan.tenant_id == integration.tenant_id]:
                external = ExternalProduct(integration_id=integration.id, plan_id=plan.id, external_id=f"price_{plan.code}_{plan.currency.lower()}", display_name=plan.name, reference_currency=plan.currency, reference_price=plan.price, active=True)
                db.add(external)
                mapped.append(external)
