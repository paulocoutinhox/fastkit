...
from enums.user import UserAddressType, UserStatus
...
async def register(page: CurrentPage, db: DatabaseSession, csrf_token: CsrfToken = None, first_name: str = Form(""), last_name: str = Form(""), email: str = Form(""), password: str = Form(""), captcha_answer: str = Form(""), captcha_token: str = Form(""), next_path: str = Form("", alias="next")):
    ...
    if user.status == UserStatus.PENDING:
        return redirect(page, "/account/login", [notice("site.sign-up-confirmation-sent")])
...
async def confirm_sign_up(page: CurrentPage, db: DatabaseSession, token: str):
    """The link is clicked in a mail client, so it is a GET, and answering it puts the person where they were going before the account existed."""
    try:
        user = await auth_service.confirm_sign_up(db, token)
    except AppError as refused:
        return redirect(page, "/account/login", [notice(refused.code, "error")])

    answer = redirect(page, landing(page.request.query_params.get("next")), [notice("site.sign-up-confirmed")])
    sign_in(answer, create_token(user.token, user.role, user.session_epoch))
...
async def resend_confirmation(page: CurrentPage, db: DatabaseSession, csrf_token: CsrfToken = None, login: str = Form(""), captcha_answer: str = Form(""), captcha_token: str = Form("")):
    ...
    return redirect(page, "/account/login", [notice("site.confirmation-resent")])
