...
from schemas.auth import AccountUpdateRequest, ConfirmationRequest, PasswordChangeRequest, PasswordResetConfirmRequest, PasswordResetRequest, TokenResponse
...
@router.post("/confirmation", status_code=status.HTTP_204_NO_CONTENT, summary="Ask for the sign up confirmation again")
async def resend_confirmation(db: DatabaseSession, brand: CurrentBrand, payload: ConfirmationRequest):
    """A login nobody has, one already confirmed and one still waiting all answer the same, so the endpoint never says who has an account here."""
    await auth_service.resend_confirmation(db, brand, payload.login)

    return Response(status_code=status.HTTP_204_NO_CONTENT)


@router.post("/confirmation/{token}", response_model=TokenResponse, summary="Confirm the address an account signed up with")
async def confirm_sign_up(db: DatabaseSession, payload_token: str = Path(alias="token", min_length=8, max_length=128)):
    """The address answered, so the account is open and this call is what puts the person straight into it."""
    user = await auth_service.confirm_sign_up(db, payload_token)
