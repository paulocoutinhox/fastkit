...
export const routes = [
    ...
    // The own account is not a resource, and the segment below matches anything, so it is declared before it or it never answers.
...
export function createAppRouter(history = createWebHistory(ADMIN_PATH)) {
    ...
    router.beforeEach(async (to) => {
        ...
        if (!meta.loaded || !permissions.loaded) {
            ...
            try {
                await Promise.all([meta.load(), permissions.load(), auth.refresh()]);
            } catch (error) {
