...
import { CATALOGS, LOCALE_STORAGE_KEY } from "@/i18n";
...
describe("AppSidebar", () => {
    ...
    it("lists every resource of a section, and only the ones a parent does not manage", async () => {
        ...
        // The two the menu carries that are not resources, and so answer to no permission: where everything starts, and the account of whoever is operating.
        // Their labels are read off the catalog rather than written here, because a copy of a label is one that outlives the label.
        const apart = [CATALOGS.en.common.dashboard, CATALOGS.en.profile.title];

        expect(wrapper.text()).toContain("Access");
        expect(wrapper.text()).toContain("Users");

        apart.forEach((name) => expect(wrapper.text()).toContain(name));

        expect(wrapper.findAll("nav a")).toHaveLength(listed.length + apart.length);
    });
