...
describe("a timer a screen started", () => {
    ...
    it("is cancelled when the screen goes, so it never asks for what nobody is looking at", () => {
        const holding = walk("src").filter((path) => /\btimer\s*=\s*setTimeout\(/.test(readFileSync(path, "utf8")));
        const leaking = holding.filter((path) => !/onBeforeUnmount\([^)]*\)?[\s\S]{0,120}clearTimeout\(timer\)/.test(readFileSync(path, "utf8")));
