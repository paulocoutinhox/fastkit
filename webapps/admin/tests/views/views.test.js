...
describe("ProfileView", () => {
    const ACCOUNT = { token: "abc", username: "ada", email: "ada@acme.com", firstName: "Ada", lastName: "Lovelace", nickname: null, gender: "none", mobilePhone: null, timezone: "UTC", avatarUrl: null, displayName: "Ada Lovelace", languageId: null };

    beforeEach(() => {
        useAuthStore().apply("abc", { ...ACCOUNT, firstName: "Stale" });
        useMetaStore().enums = { user_gender: ["none", "female", "male"] };
    });

    it("is what the address answers, and not the resource the next route matches anything with", () => {
        expect(createTestRouter("/profile").resolve("/profile").name).toBe("profile");
    });

    it("fills itself from what the account answers now, and not from the snapshot sign in stored", async () => {
        vi.spyOn(api, "get").mockResolvedValue(ACCOUNT);

        const wrapper = mountView(ProfileView, createTestRouter("/profile"));

        await flushPromises();

        expect(api.get).toHaveBeenCalledWith("/account/me");
        expect(wrapper.find("#field-firstName").element.value).toBe("Ada");
    });

    it("writes the own account and hands the answer to what draws the name", async () => {
        vi.spyOn(api, "get").mockResolvedValue(ACCOUNT);

        const saved = vi.spyOn(api, "put").mockResolvedValue({ ...ACCOUNT, firstName: "Grace", displayName: "Grace Lovelace" });
        const wrapper = mountView(ProfileView, createTestRouter("/profile"));

        await flushPromises();
        await wrapper.find("#field-firstName").setValue("Grace");
        await wrapper.findAll("form")[0].trigger("submit");
        await flushPromises();

        expect(saved).toHaveBeenCalledWith("/account/me", expect.objectContaining({ firstName: "Grace", timezone: "UTC" }));
        expect(useAuthStore().user.displayName).toBe("Grace Lovelace");
    });

    it("points at the field the api refused", async () => {
        vi.spyOn(api, "get").mockResolvedValue(ACCOUNT);
        vi.spyOn(api, "put").mockRejectedValue({ message: "", errors: { email: "That address is already taken." } });
    ...
    it("takes the token the password change answered and leaves what the account reaches alone", async () => {
        const permissions = usePermissionsStore();
        permissions.reachable = ["tenants"];
        permissions.loaded = true;

        vi.spyOn(api, "get").mockResolvedValue(ACCOUNT);
        vi.spyOn(api, "post").mockResolvedValue({ token: "minted", user: ACCOUNT });

        const wrapper = mountView(ProfileView, createTestRouter("/profile"));

        await flushPromises();
        await wrapper.find("#field-currentPassword").setValue("s3cret-password");
        await wrapper.find("#field-newPassword").setValue("another-password");
        await wrapper.findAll("form")[1].trigger("submit");
        await flushPromises();

        expect(api.post).toHaveBeenCalledWith("/account/password", { currentPassword: "s3cret-password", newPassword: "another-password" });
        expect(useAuthStore().token).toBe("minted");

        // Signing out and back in is what empties this, and changing a password is neither.
        expect(permissions.loaded).toBe(true);
        expect(wrapper.find("#field-currentPassword").element.value).toBe("");
    });

    it("sends the picture to the one address that stores it and never holds a storage key", async () => {
        vi.spyOn(api, "get").mockResolvedValue(ACCOUNT);

        const sent = vi.spyOn(api, "post").mockResolvedValue({ ...ACCOUNT, avatarUrl: "/media/images/user/avatar/a.webp" });
        const wrapper = mountView(ProfileView, createTestRouter("/profile"));

        await flushPromises();

        const input = wrapper.find("#account-picture");
        Object.defineProperty(input.element, "files", { value: [new File(["x"], "face.png", { type: "image/png" })] });

        await input.trigger("change");
        await flushPromises();

        expect(sent.mock.calls[0][0]).toBe("/account/avatar");
        expect(sent.mock.calls[0][1]).toBeInstanceOf(FormData);
        expect(useAuthStore().user.avatarUrl).toBe("/media/images/user/avatar/a.webp");
    });
});
