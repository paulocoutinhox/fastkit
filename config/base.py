...
from enums.upload import ImageFormat, Naming, UploadPurpose
...
# The same name where it has to be an identifier rather than a title: every cookie this site writes, and the throwaway container the schema diff builds in.
# It is derived so that a product started from this template carries its own name on day one instead of writing somebody else's on the browsers of its readers.
SLUG = slugify(NAME)
...
class EmailSettings(BaseModel):
    """Which service carries the mail of this environment, and the account it sends through."""
...
class CaptchaSettings(BaseModel):
    ...
    provider: CaptchaProvider = CaptchaProvider.IMAGE
    site_key: str = ""
    secret_key: str = ""
...
class SiteSettings(BaseModel):
    ...
    session_cookie: str = f"{SLUG}_session"
    flash_cookie: str = f"{SLUG}_flash"

    # What proves a form was drawn here, named beside the other six so that no cookie of this site is spelled anywhere else.
    csrf_cookie: str = f"{SLUG}_csrf"

    # The language a visitor chose, which is where it lives for somebody with no account to keep it in.
    language_cookie: str = f"{SLUG}_language"
    language_max_age: int = 60 * 60 * 24 * 365

    # The palette a visitor chose, kept the same way and for the same reason as the language.
    theme_cookie: str = f"{SLUG}_theme"
    ...
    visitor_cookie: str = f"{SLUG}_visitor"
...
class Settings(BaseModel):
    ...
    # Whether an address proves it is the address before the account it opened answers, which is what stops somebody signing up with an inbox that is not theirs.
    confirm_sign_up: bool = True

    # How long an address waits between two confirmation mails, for the same reason the recovery has one: the route is open.
    confirm_sign_up_interval: int = 60
